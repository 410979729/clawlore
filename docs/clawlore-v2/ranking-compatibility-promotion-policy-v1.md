...
For the compatibility window, a separately approved one-time backfill may read
the already-derived V1 `memory_truth.metadata_text`; it must not copy the raw
...
| --- | --- | --- |
| Manual/private | trusted runtime or registry-direct attribution, matching principal, evidence digest, user/operator confirmation | eligible for separate operator promotion review |
| Manual/conversation | registry-conversation attribution, matching conversation boundary and digest, user/operator confirmation | eligible for separate operator promotion review |
| Task experience | trusted identity/boundary, tool/operator verification, at least one source receipt | eligible for separate operator promotion review |
| Reflection/checkpoint/auto-capture | trusted identity/boundary, explicit operator review id, operator verification, at least one receipt | eligible for separate operator promotion review |
...
Even an eligible row still needs a new operator approval bound to an exact
item-digest list and rollback receipt. Eligibility is not authorization.
...
6. a separate operator approval for the exact projection and/or promotion digest;
