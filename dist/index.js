...
import { detectCategory, shouldCapture, } from "./src/auto-capture-policy.js";
            ...
            const agentEndAutoCaptureHook = (event, ctx) => {
                ...
                const backgroundRun = (async () => {
                    try {
                        ...
                        const { eligibleTexts, pendingIngressCount, skippedTextCount: skippedAutoCaptureTexts, } = captureSelection;
                        ...
                        if (pendingIngressCount > 0) {
                            api.logger.debug(`clawlore: auto-capture using ${pendingIngressCount} pending ingress text(s) for agent=${diagnosticIdentifier(agentId)}`);
                        }
