...
function extractFeatures(text) {
    ...
    for (const token of normalized.match(/[a-z0-9][a-z0-9_.:+-]*/gu) ?? []) {
        if (token.length < 2 || LATIN_STOP_WORDS.has(token))
            continue;
        const feature = `latin:${token}`;
        values.add(feature);
        if (token.length >= 5 || /[0-9_.:+-]/u.test(token))
            symbolic.add(feature);
    }
    for (const segment of normalized.match(/[\u3400-\u9fff]+/gu) ?? []) {
        for (const width of [2, 3]) {
            for (let index = 0; index + width <= segment.length; index += 1) {
                ...
                if (!CJK_STOP_FEATURES.has(token))
                    values.add(`cjk:${token}`);
            }
        }
    }
 ...
 */
export function rankManualRecallLexicalEvidence(query, candidates) {
    const queryFeatures = extractFeatures(query);
    const asksWho = /(?:谁|\bwho\b)/iu.test(query);
    const candidateFeatures = candidates.map((candidate) => extractFeatures(candidate.entry?.text ?? ""));
    ...
    return candidates.map((candidate, index) => {
        ...
        const answerShapeBonus = asksWho && ((candidateText.match(/、/gu)?.length ?? 0) >= 2
            || (candidateText.match(/[,，;；]/gu)?.length ?? 0) >= 3) ? 0.12 : 0;
        const rankingScore = clamp01((candidate.score * 0.55) + (lexicalCoverage * 0.45) + answerShapeBonus);
        return {
            candidate,
            lexicalCoverage,
            matchedFeatureCount,
