...
function explicitOutcomeFromObject(value, depth = 0, seen = new Set()) {
    ...
    if (typeof obj.code === "number" && Number.isInteger(obj.code)) {
        if (obj.code === 0 || (obj.code >= 200 && obj.code < 300))
            sawSuccess = true;
        else if ((obj.code > 0 && obj.code < 100) || obj.code >= 400)
            sawFailure = true;
    }
 ...
 */
export function structuredToolOutcome(message) {
    if (!isToolResultMessage(message))
        return null;
    return explicitOutcomeFromObject(message);
}
export function summarizeStructuredToolOutcomes(messages) {
    let resultCount = 0;
    let successCount = 0;
    let failureCount = 0;
...
export function agentEndEventAllowsTaskExperience(event) {
    ...
    for (const key of ["status", "outcome"]) {
        if (FAILURE_STATES.has(normalizedState(obj[key])))
            return false;
    }
    return true;
}
...
function sentenceStart(value, offset) {
    let boundary = -1;
 ...
 */
function hasNonProtectiveCapabilityDenial(value) {
    ...
    for (const match of matches) {
        ...
        const protective = hasProtectiveSubject && !hasSubstantiveSubject
            ? true
...
export function finalAssistantClaimsVerifiedSuccess(text) {
...
export function finalAssistantLooksUnsuccessful(text) {
    ...
    if (!final)
        return false;
    if (hasNonProtectiveCapabilityDenial(final))
        return true;
    const positive = [
