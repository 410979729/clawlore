...
export function parseOutboundEndpointPolicy(value) {
    if (value === undefined)
    ...
    const values = (raw.allowedPrivateHosts ?? []);
...
export function validateOutboundEndpointSyntax(value, policy) {
    let url;
    try {
        url = new URL(value);
    }
...
async function resolveAllowedAddresses(hostname, policy, resolveHost = async (hostname) => lookup(hostname, { all: true, verbatim: true })) {
    if (isIP(hostname) !== 0) {
    ...
    if (!policy.allowedPrivateHosts.includes(hostname)
        && normalizedAddresses.some((entry) => !isPublicAddress(entry.address))) {
        throw new Error("CLAWLORE_OUTBOUND_ENDPOINT_PRIVATE_ADDRESS_BLOCKED");
    }
    return normalizedAddresses;
}
export async function assertOutboundEndpointAllowed(value, policy, resolveHost = async (hostname) => lookup(hostname, { all: true, verbatim: true })) {
    const url = validateOutboundEndpointSyntax(value, policy);
    const hostname = normalizeHost(url.hostname);
    await resolveAllowedAddresses(hostname, policy, resolveHost);
    return url;
}
function createValidatedLookup(policy, resolveHost) {
    return (hostname, options, callback) => {
        ...
        resolveAllowedAddresses(normalizedHostname, policy, resolveHost).then((resolved) => {
            const addresses = [...new Map(resolved.map((entry) => [entry.address, entry])).values()].filter((entry) => entry.family !== 0
            ...
            callback(failure, options.all ? [] : "", 0);
        });
    };
}
/**
 ...
 */
export function createSafeOutboundFetch(policy, dependencies = {}) {
    ...
    return async (input, init) => {
        ...
        if (dependencies.fetch) {
            ...
            response = await undiciFetch(input, requestInit);
        }
