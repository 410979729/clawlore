...
export function diagnosticTextSummary(value) {
    const text = typeof value === "string" ? value.trim() : "";
    return `len=${text.length},hash=${diagnosticHash(text)}`;
}
export function diagnosticContentSummary(content) {
    if (typeof content === "string")
        return `string(${diagnosticTextSummary(content)})`;
...
export function diagnosticErrorSummary(error) {
