 ...
 */
export function normalizeStructuredSecretKey(value) {
...
export function isStructuredSecretKey(value) {
    if (CJK_SECRET_KEYS.has(unquoteKey(value)))
        return true;
    ...
    if (SECRET_KEY_SUFFIXES.has(finalPart))
        return true;
...
function collectYamlSpans(text) {
    const spans = [];
    let documents;
    try {
...
function readYamlBlockValue(text, start, keyOffset) {
    ...
    while (cursor < text.length) {
        ...
        if (line.trim() && leadingIndent(text, cursor) <= keyIndent)
            break;
        end = lineEnd;
        cursor = lineEnd;
    }
...
function readFallbackValue(text, start, keyOffset) {
    ...
    while (cursor < text.length && !/[\r\n,;\]}]/.test(text[cursor] ?? ""))
        cursor++;
    while (cursor > start && /\s/.test(text[cursor - 1] ?? ""))
        cursor--;
    return cursor;
}
function collectXmlAttributeSpans(text) {
    const spans = [];
    ...
    while ((tagMatch = tag.exec(text)) !== null) {
        ...
        const semanticKey = attributes.find((candidate) => ["key", "name"].includes(normalizeStructuredSecretKey(candidate.name))
...
function collectNpmrcSpans(text) {
    const spans = [];
    ...
    while ((match = assignment.exec(text)) !== null) {
        ...
        if (!isStructuredSecretKey(key) && normalizedKey !== "auth")
            continue;
 ...
 */
export function findStructuredSecretSpans(text) {
    const spans = [
    ...
    while ((match = assignment.exec(text)) !== null) {
        ...
        if (end > start)
            spans.push({ key: unquoteKey(key), start, end, value: text.slice(start, end) });
        if (match[0].length === 0)
            assignment.lastIndex += 1;
    }
