...
function isMetadataStringSafe(value) {
    const normalized = value.trim();
    if (sanitizeCaptureText(value) !== normalized)
        return false;
...
function visitMetadata(value, depth, budget) {
    budget.nodes += 1;
    ...
    if (typeof value === "string")
        return isMetadataStringSafe(value);
    ...
    if (prototype !== Object.prototype && prototype !== null)
        return false;
    return Object.entries(value).every(([key, item]) => key.length <= 512 && isMetadataStringSafe(key)
 ...
 * standalone memories.
 */
export function isMemoryMetadataSafe(metadata) {
    let parsed = metadata;
...
export function assertMemoryMetadataSafe(metadata) {
