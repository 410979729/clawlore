import { parseLifecycleMetadata, staticLifecycleForMetadata, } from "./smart-metadata.js";
...
function projectionFingerprintExpression(prefix = "") {
    return `json_array(
    ${prefix}scope,
...
function expectedStaticLifecycleExpression(metadataSql, timestampSql) {
...
function objectType(db, name) {
    const row = db.prepare("SELECT type FROM sqlite_master WHERE name = ?").get(name);
    return String(row?.type || "");
}
function columnsMatch(db, table, expected) {
    const columns = db.prepare(`PRAGMA table_info(${table})`).all()
        .map((row) => row.name);
    return columns.length === expected.length
        && expected.every((column, index) => columns[index] === column);
}
function projectionSchemaCompatible(db) {
    if (objectType(db, "memory_lifecycle_projection") !== "table")
        return false;
    if (objectType(db, "memory_lifecycle_projection_state") !== "table")
        return false;
    if (objectType(db, "idx_memory_lifecycle_projection_scope") !== "index")
        return false;
    if (!columnsMatch(db, "memory_lifecycle_projection", PROJECTION_COLUMNS))
        return false;
    if (!columnsMatch(db, "memory_lifecycle_projection_state", PROJECTION_STATE_COLUMNS))
        return false;
    const indexColumns = db.prepare("PRAGMA index_info(idx_memory_lifecycle_projection_scope)").all()
        .map((row) => row.name);
...
function valuesFor(entry, truthUpdatedAt) {
...
function registerLifecycleFieldFunction(db) {
    if (typeof db.function !== "function") {
        throw new Error("CLAWLORE_LIFECYCLE_PROJECTION_HEALTH: SQLite scalar functions unavailable");
    }
    db.function(LIFECYCLE_FIELD_FUNCTION, { deterministic: true }, (truthMetadata, truthTimestamp, field) => {
...
function writeProjection(db, values) {
    db.prepare(`
  ...
  `).run(values.memoryId, values.scope, values.staticLifecycle, values.validFrom, values.invalidatedAt, values.truthUpdatedAt, values.scope, values.staticLifecycle, values.validFrom, values.invalidatedAt, values.invalidatedAt, values.truthUpdatedAt);
}
function rebuildProjection(db) {
...
function dropObjectIfPresent(db, name) {
...
export function inspectLifecycleProjection(db) {
    ...
    const projectedRows = Number(db.prepare("SELECT COUNT(*) AS count FROM memory_lifecycle_projection").get()?.count || 0);
...
export function repairLifecycleProjection(db) {
    ...
    try {
        ...
        try {
            db.exec("ROLLBACK TO SAVEPOINT clawlore_lifecycle_projection_rebuild");
        }
        catch { }
        try {
            db.exec("RELEASE SAVEPOINT clawlore_lifecycle_projection_rebuild");
...
export function ensureLifecycleProjection(db, options = {}) {
    const health = inspectLifecycleProjection(db);
    if (options.force === true || !health.ok)
        return repairLifecycleProjection(db);
    return health;
}
export function upsertLifecycleProjection(db, entry, truthUpdatedAt) {
    const existed = Boolean(db.prepare("SELECT 1 FROM memory_lifecycle_projection WHERE memory_id = ?").get(entry.id));
...
export function deleteLifecycleProjection(db, memoryId) {
    const removed = Number(db.prepare("DELETE FROM memory_lifecycle_projection WHERE memory_id = ?").run(memoryId)?.changes || 0);
    if (removed > 0) {
        db.prepare(`
      UPDATE memory_lifecycle_projection_state
...
export function syncLifecycleProjectionFromTruth(db, memoryId) {
...
function readLifecycleScopeCounts(db, input) {
 ...
 */
export function openLifecycleProjectionReadAccess(db) {
    const health = inspectLifecycleProjection(db);
    return {
        health,
