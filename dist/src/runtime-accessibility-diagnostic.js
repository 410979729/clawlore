import { isCanonicalPrincipalKey, normalizePrincipalIsolationConfig, runtimePrincipalIdentity, } from "./runtime-memory-boundary.js";
function safeCount(scopeCounts, scope) {
    const value = scopeCounts[scope];
    return Number.isFinite(value) && value > 0 ? Math.trunc(value) : 0;
}
function lifecycleCount(stats, scopeCounts, scope, lifecycle) {
    if (!stats?.[scope])
        return lifecycle === "recallable" ? safeCount(scopeCounts, scope) : 0;
...
export function assessRuntimeAccessibility(input) {
    ...
    const principalIdentity = input.principalKey
        ...
        : undefined;
    const legacyScopeAccessible = Boolean(input.principalKey
    ...
    const accessibleScopes = principalIdentity
        ...
        : [];
    const visibleRows = accessibleScopes.reduce((total, scope) => total + lifecycleCount(input.lifecycleScopeCounts, input.scopeCounts, scope, "recallable"), 0);
    const archivedRows = accessibleScopes.reduce((total, scope) => total + lifecycleCount(input.lifecycleScopeCounts, input.scopeCounts, scope, "archived"), 0);
    let status = "ready";
    let blocking = false;
