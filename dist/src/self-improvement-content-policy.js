...
function escapeStructuredLines(value) {
...
export function normalizeSelfImprovementSummary(value) {
...
export function normalizeSelfImprovementBody(value, label, maxLength, fallback = "-") {
    const raw = requireBoundedString(value, label, maxLength).trim();
    if (!raw)
        return fallback;
    const safe = redactMemoryTextForOutput(raw);
    if (!safe || safe === REDACTED_MEMORY_CONTENT)
        return REDACTED_UNSAFE_CONTENT;
    return escapeStructuredLines(safe);
}
export function normalizeSelfImprovementLabel(value, label, fallback) {
