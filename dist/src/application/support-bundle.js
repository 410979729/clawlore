...
function redactSupportBundleValue(value, key, ancestors, depth) {
    if (SENSITIVE_KEY.test(key) || isStructuredSecretKey(key))
        return "<redacted-secret>";
    ...
    if (value && typeof value === "object") {
        ...
        if (entries.length > MAX_COLLECTION_ITEMS)
            result.__truncated_entries__ = entries.length - MAX_COLLECTION_ITEMS;
        ancestors.delete(value);
        return result;
    }
    return value;
...
export function redactSupportBundle(value, key = "") {
    return redactSupportBundleValue(value, key, new Set(), 0);
}
