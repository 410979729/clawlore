...
import { RELEASE_IMMUTABLE_BINDING_FIELDS_V1, validateRuntimeReleaseReadinessV1, } from "./application/runtime-release-readiness-validation.js";
...
function tableExists(db) {
    return Number(db.prepare("SELECT COUNT(*) AS n FROM sqlite_master WHERE type='table' AND name=?").get(AUTHORITY_TABLE).n) === 1;
}
function schemaIsValid(db) {
    const columns = db.prepare(`PRAGMA table_info(${AUTHORITY_TABLE})`).all();
    if (columns.length !== AUTHORITY_COLUMNS.size)
        return false;
    for (const column of columns) {
        ...
        if (!expectedType || String(column.type).toUpperCase() !== expectedType)
            return false;
        if (column.name === "singleton" && Number(column.pk) !== 1)
            return false;
    }
    return true;
}
...
function inspectDatabase(db, expectedBinding, expectedMode) {
    if (!tableExists(db)) {
    ...
    const rows = db.prepare(`SELECT * FROM ${AUTHORITY_TABLE} ORDER BY singleton`).all();
    ...
    const mismatchedFields = [
        ...
        ...RELEASE_IMMUTABLE_BINDING_FIELDS_V1.filter((field) => authorityBinding[field] !== expectedBinding[field]),
    ];
    return {
        schemaVersion: 1,
...
export function runtimeReleaseReadinessDigestV1(readiness) {
...
export function recordRuntimeReleaseAuthorityV1(input) {
    ...
    try {
      ...
      `).run(input.expectedMode, input.expectedBinding.sourceCommit, input.expectedBinding.runtimeDigest, input.expectedBinding.packageDigest, input.expectedBinding.lockDigest, input.expectedBinding.configDigest, input.expectedBinding.testLogDigest, input.expectedBinding.truthSnapshotDigest, readinessSha256, readiness.provenance.createdAt, readiness.provenance.expiresAt, now.toISOString());
            db.exec("COMMIT");
        }
        catch (error) {
            try {
