...
function collectReflectionChunks(db, scope, maxChunks) {
...
function memoryCategoryFor(type) {
    switch (type) {
        case "decision":
            return "decision";
        case "preference":
...
function normalizeCandidate(raw, fallbackEvidence) {
    ...
    if (!memoryType || abstract.length < 12 || content.length < 20)
        return null;
    const candidate = {
...
function classifyHeuristic(text) {
    ...
    if (/(坑|报错|失败|错误|回归|pitfall|bug|failed|error|regression)/i.test(text))
        return "pitfall";
    if (/(流程|步骤|sop|playbook|workflow|run .*verify|verify .*run|release gate)/i.test(text))
        return "workflow";
    if (/(决定|决策|必须|禁止|不能|should|must|decision|decided)/i.test(text))
        return "decision";
    ...
    if (/https?:\/\/|path:|文件|文档|resource/i.test(text))
        return "resource";
    if (lowered.includes("case") || lowered.includes("事故") || lowered.includes("复盘"))
        return "case";
    return null;
}
function heuristicCandidate(chunk) {
...
async function llmCandidates(chunk, llmClient) {
...
${chunk.text.slice(0, 5000)}`;
    const response = await llmClient.completeJson(prompt, "digest-extraction");
...
function candidateToEntry(candidate, chunk, vector) {
    const text = [
        candidate.abstract,
        "",
        candidate.overview,
...
function insertRun(db, row) {
  ...
  `).run(row.id, row.run_date, row.started_at, row.completed_at, row.status, row.source_type, row.source_count, row.chunk_count, row.candidate_count, row.stored_count, row.skipped_count, row.error_count, row.notes, row.actor);
}
function updateRun(db, row) {
    db.prepare(`
...
function runStatusFrom(params) {
    if (params.chunkCount === 0)
        return "empty";
    if (params.llmParseErrors > 0 && params.candidateCount === 0 && params.errors.length > 0)
        return "parse_error";
    if (params.candidateCount === 0 && params.skipped > 0)
        return "filtered";
    if (params.errors.length > 0 && params.candidateCount === 0)
...
function validateCandidate(candidate) {
    ...
    if (!safety.allowed)
        return { ok: false, reason: safety.reason || "capture_safety" };
    if (isNoise(candidate.content) || isNoise(candidate.abstract))
        return { ok: false, reason: "noise_filter" };
    if (candidate.content.length < 20 || candidate.abstract.length < 12)
        return { ok: false, reason: "too_short" };
    return { ok: true };
}
export async function runDigestPipeline(db, options = {}) {
...
export function digestReport(db, options = {}) {
...
export function digestRecoveryReport(db, options = {}) {
...
export function recoverDigestChunks(db, options = {}) {
