...
export function parseScopeDimensions(scope) {
    const raw = cleanToken(scope);
    if (!raw)
        return { raw: "", kind: "unknown", id: "" };
    if (raw === "global")
        return { raw, kind: "global", id: "global" };
...
export function scopeIdForContext(context) {
    if (context.scope_id && cleanToken(context.scope_id))
        return cleanToken(context.scope_id);
    if (context.customer_host)
        return `custom:customer:${cleanToken(context.customer_host)}`;
    if (context.project_id)
        return `project:${cleanToken(context.project_id)}`;
    if (context.channel_id)
        return `custom:channel:${cleanToken(context.channel_id)}`;
    if (context.task_class)
        return `custom:task:${cleanToken(context.task_class)}`;
    if (context.agent_id)
        return `agent:${cleanToken(context.agent_id)}`;
    return "global";
}
function sameBoundary(current, candidate) {
...
export function evaluateRecallScopePolicy(input) {
