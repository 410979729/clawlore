...
// ============================================================================
function detectRiskLevel(evidence, toolNames, goal) {
...
// ============================================================================
function detectVerification(evidence, toolNames) {
...
export function promoteExperiences(db, options = {}) {
    ...
    for (const episode of episodes) {
        ...
        const existingPlaybook = db.prepare("SELECT id FROM procedural_playbooks WHERE created_from_episode_id = ?").get(episode.id);
        if (existingPlaybook) {
        ...
        if (riskLevel === "low" && config.auto_promote_low_risk) {
            updatePlaybookStatus(db, created.id, "promoted", "自动提取经验自检通过：低风险且有验证证据。");
            finalStatus = "promoted";
            result.playbooks_promoted++;
        }
        else if (riskLevel === "high") {
            updatePlaybookStatus(db, created.id, "needs_review", "自动提取经验自检发现高风险动作；需要后续代理复核。");
 ...
 */
export function runForgettingLoop(db, options = {}) {
    const dryRun = options.dry_run ?? true;
    const result = {
        dry_run: dryRun,
        candidates_found: 0,
