...
import { readSessionConversationWithResetFallback, redactReflectionText, summarizeRecentConversationMessages, } from "./reflection-transcript.js";
...
export function registerReflectionHooks(params) {
    ...
    if (config.sessionStrategy === "memoryReflection") {
        ...
        api.on("after_tool_call", (event, ctx) => {
            ...
            if (isInternalSession(sessionKey) || !sessionKey)
                return;
            state.prune();
            const addSignal = (text, source) => {
            ...
            if (text && containsErrorSignal(text))
                addSignal(text, "tool_output");
        }, { priority: 15 });
        api.on("before_prompt_build", async (event, ctx) => {
            const sessionKey = typeof ctx.sessionKey === "string" ? ctx.sessionKey : "";
            if (isInternalSession(sessionKey))
                return;
            if (injectMode !== "inheritance-only" && injectMode !== "inheritance+derived")
                return;
            try {
                ...
                if (access.denied)
                    return;
                state.prune();
                const agentId = params.resolveAgentId(typeof ctx.agentId === "string" ? ctx.agentId : undefined, sessionKey);
    ...
    if (config.sessionStrategy === "systemSessionMemory") {
        ...
        const getGuard = () => {
            ...
            if (!globalRecord[guardSymbol])
                globalRecord[guardSymbol] = new Map();
            return globalRecord[guardSymbol];
        };
        const pruneGuard = (now) => {
            for (const [key, storedAt] of getGuard()) {
                if (now - storedAt > guardTtlMs)
                    getGuard().delete(key);
            }
        };
        const storeSummary = async (input) => {
            ...
            await store.store({
                ...
                importance: 0.5,
                metadata: stringifySmartMetadata(buildSmartMetadata({ text: `Session summary for ${date}`, category: "fact", importance: 0.5, timestamp: Date.now() }, {
