...
function renderSection(section, budget) {
    ...
    if (budget <= heading.length)
        return truncate(heading, budget);
 ...
 */
export function formatTaskExperienceCapsule(fields, maxChars) {
    ...
    const criticalTotal = Math.min(available, Math.max(minimumCritical, Math.floor(available * 0.62)));
...
function metadataFields(metadata) {
    ...
    if (!taskType || !structuredKeys.some((key) => Array.isArray(metadata[key])))
        return null;
    return {
        taskType,
...
export function formatTaskExperienceRecallCapsule(metadata, fallbackText, maxChars) {
