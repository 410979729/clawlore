...
function tableExists(db, name) {
    return Number(db.prepare("SELECT COUNT(*) AS n FROM sqlite_master WHERE type='table' AND name=?").get(name).n) === 1;
}
 ...
 */
export function ensureFreshInstallV2AuthorityV1(sqlitePath) {
    ...
    try {
        ...
        if (hasAuthority) {
            const row = inspection.prepare("SELECT authority FROM clawlore_runtime_authority WHERE singleton=1").get();
