 ...
 */
export function evaluateMemoryMergePayload(input) {
    ...
    if (value.abstract.length > MAX_ABSTRACT_CHARS ||
    ...
    const safety = evaluateCaptureSafety([value.abstract, value.overview, value.content].filter(Boolean).join("\n"));
    if (!safety.allowed) {
