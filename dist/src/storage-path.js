...
export function validateStoragePath(dbPath) {
    ...
    try {
        ...
        if (stats.isSymbolicLink()) {
            try {
                ...
                throw new Error(`CLAWLORE_STORAGE_PATH_INVALID: path=${diagnosticIdentifier(dbPath)} dangling_symlink ${diagnosticErrorSummary(error)}`);
            }
        }
    }
    catch (error) {
    ...
    try {
        ...
        throw new Error(`CLAWLORE_STORAGE_PRIVATE_DIRECTORY_REQUIRED: path=${diagnosticIdentifier(resolvedPath)} parent=${diagnosticIdentifier(dirname(resolvedPath))} ${diagnosticErrorSummary(error)}`);
    }
    try {
        accessSync(resolvedPath, constants.W_OK);
    }
    catch (error) {
        throw new Error(`CLAWLORE_STORAGE_NOT_WRITABLE: path=${diagnosticIdentifier(resolvedPath)} ${diagnosticErrorSummary(error)}`);
    }
    return resolvedPath;
}
