...
export function searchPlaybooks(db, filters) {
    ...
    if (filters.query) {
      ...
      LIMIT ?
    `).all(safeFtsQuery, ...values, limit);
        return rows.map((row) => {
            const playbook = rowToPlaybook(row);
            return { ...playbook, score: -Number(row.fts_rank ?? 0) };
        }).sort((a, b) => (b.score ?? 0) - (a.score ?? 0));
    }
    const rows = db.prepare(`SELECT p.* FROM procedural_playbooks p ${where} ORDER BY p.confidence DESC, p.updated_at DESC LIMIT ?`).all(...values, limit);
...
export function updatePlaybookStatus(db, playbookId, status, reason, scopeIds) {
    ...
    `).run(status, now, playbookId, ...scope.params);
        ...
        if (!durable)
            throw new Error("playbook status update lost its durable row");
        createPlaybookVersion(db, playbookId, currentVersion + 1, "status_change", reason ?? `Status changed to ${status}`, durable);
    });
}
...
export function recordPlaybookFeedbackAtomically(db, params) {
