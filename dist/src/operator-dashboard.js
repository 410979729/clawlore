...
function groupedCounts(db, table, column) {
    try {
        const rows = db.prepare(`SELECT ${column} AS key, COUNT(*) AS count FROM ${table} GROUP BY ${column}`).all();
...
function experienceHealth(db, tables) {
    const required = [
        "task_episodes",
    ...
    if (missing.length > 0)
        return { status: "not_initialized", missing_tables: missing };
    return {
        status: "ready",
...
function freshnessHealth(db, tables) {
  ...
  `).all();
    ...
    for (const row of rows) {
        ...
        if (needsLiveCheck)
            liveCheckNeeded++;
        if (isUnknown)
            unknownFreshnessFacts++;
        if (isStale || needsLiveCheck || isUnknown)
            debt++;
        if (samples.length < 10) {
            if (isStale)
                samples.push({ id: row.id, reason: "stale" });
            else if (needsLiveCheck)
                samples.push({ id: row.id, reason: "live_check_needed" });
            else if (isUnknown)
                samples.push({ id: row.id, reason: "unknown_freshness" });
        }
    }
    return {
...
export function buildOperatorDashboard(db, options = {}) {
