...
export class Embedder {
    ...
    async embedWithRetry(payload, signal) {
        ...
        throw new Error(`All ${maxAttempts} API keys exhausted (rate limited). Last error: ${diagnosticErrorSummary(lastError)}`);
    }
...
export class MiniMaxEmbedder {
    ...
    async request(texts, type, signal) {
        const endpoint = new URL(`${this._baseURL}/embeddings`);
        if (this._groupId)
            endpoint.searchParams.set("GroupId", this._groupId);
        ...
        for (let attempt = 0; attempt < this._apiKeys.length; attempt++) {
            ...
            if (!response.ok) {
                ...
                if (this.isRateLimitStatus(response.status) && attempt < this._apiKeys.length - 1)
                    continue;
                throw lastError;
            }
            const data = await response.json();
            const vectors = Array.isArray(data.vectors)
                ? data.vectors
                : Array.isArray(data.data?.vectors)
                    ? data.data.vectors
                    : undefined;
            if (!vectors) {
            ...
            return vectors.map((vector) => {
                if (!Array.isArray(vector))
                    throw new Error("MiniMax embedding vector is not an array");
    ...
    withTimeout(promiseFactory, externalSignal) {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), EMBED_TIMEOUT_MS);
        let unsubscribe;
        if (externalSignal) {
