...
function validRolloutId(value) {
    return /^[a-z0-9][a-z0-9._-]{7,127}$/i.test(value);
}
function exactSearchAllowlist(values) {
...
export function buildPhase7GControlBundleV1(input) {
    ...
    if (!validDigest(input.snapshot.receiptSha256))
        blockers.push("snapshot_receipt_digest_invalid");
    if (!validDigest(input.snapshot.sourceLogicalDigest))
        blockers.push("snapshot_source_digest_invalid");
    if (!validCount(input.snapshot.sourceRows))
        blockers.push("snapshot_source_rows_invalid");
    ...
    if (!Number.isFinite(snapshotTime) || snapshotTime > now)
        blockers.push("snapshot_timestamp_invalid");
    else if (ageSeconds > maximumAgeSeconds)
        blockers.push("fresh_encrypted_snapshot_required");
    if (input.snapshot.restoreVerified !== true)
        blockers.push("snapshot_restore_verification_missing");
    if (input.snapshot.sourceUnchanged !== true)
        blockers.push("snapshot_source_changed");
    if (input.snapshot.plaintextResidueFiles !== 0)
        blockers.push("snapshot_plaintext_residue_present");
    ...
    if (!validCount(compatibility.sourceRows)
    ...
    if (compatibility.sourceRows !== input.snapshot.sourceRows
    ...
    if (compatibility.existingProjectionRows !== 0)
        blockers.push("compatibility_projection_already_exists");
    if (compatibility.rawLegacyMetadataCopied !== false)
        blockers.push("raw_legacy_metadata_copy_forbidden");
    ...
    if (promotionCount !== promotion.rows.length)
        blockers.push("promotion_plan_counts_inconsistent");
    if (promotionCount !== input.snapshot.candidateRows
