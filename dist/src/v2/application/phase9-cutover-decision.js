export function decideClawLorePhase9CutoverV1(input) {
    const blockers = [];
    if (input.live.activeInjectableRows === 0 || input.live.activeRows === 0) {
        blockers.push("no_active_verified_memory");
    }
    if (input.promotion.eligibleRows === 0)
        blockers.push("no_eligible_candidate_promotion");
    if (!input.promotion.lifecycleRolloutSelectable)
        blockers.push("candidate_lifecycle_rollout_not_selectable");
    if (input.live.candidateRows > 0 && input.live.candidateUnverifiedRows > 0) {
        blockers.push("candidate_verification_debt_present");
    }
    if (input.phase8g.proposedSoftArchiveRows > 0)
        blockers.push("phase8g_archive_proposals_unapplied");
    if (input.live.contentDivergenceRows > 0)
        blockers.push("v1_v2_current_content_divergence_present");
    if (!input.runtime.cutoverModeImplemented)
        blockers.push("runtime_cutover_mode_not_implemented");
    if (input.runtime.configuredMode !== "shadow"
