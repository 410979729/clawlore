...
function validateSpecification(specification) {
    for (const digest of [
        ...
        if (!isDigest(digest))
            throw new Error("durable rewrite specification digest is invalid");
    ...
    ].includes(specification.factKey))
        throw new Error("durable rewrite fact key is invalid");
    if (![
        "covered_by_existing_truth",
        "materially_new_bounded_truth",
    ].includes(specification.knowledgeCoverage))
        throw new Error("durable rewrite knowledge coverage is invalid");
    if (specification.proposedContent !== specification.proposedContent.trim()
...
export function planCandidateDurableRewriteProposalV1(rows, specifications, corpusContents) {
    ...
    for (const row of rewriteRows) {
        if (row.basis !== "durable_fact_requires_rewrite"
        ...
        for (const digest of [
            ...
            if (!isDigest(digest))
                throw new Error("durable rewrite row digest is invalid");
    ...
    for (const specification of specifications) {
        ...
        if (factKeys.has(specification.factKey))
            throw new Error("durable rewrite fact keys must be unique");
    ...
    for (const [normalizedContentDigest, group] of [...groups].sort(([left], [right]) => left.localeCompare(right))) {
        ...
        if (!safety.allowed)
            throw new Error("durable rewrite proposal still contains capture-unsafe trace material");
    ...
    for (const digest of specsByDigest.keys()) {
        if (!groups.has(digest))
            throw new Error("durable rewrite specification targets an unknown group");
    }
    const plannedByDigest = new Map(plannedGroups.map((group) => [group.normalizedContentDigest, group]));
    const plannedRows = rewriteRows.map((row) => {
