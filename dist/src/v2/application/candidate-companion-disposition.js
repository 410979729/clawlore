...
export function digestCandidateCompanionDispositionV1(value) {
    return createHash("sha256").update(JSON.stringify(value)).digest("hex");
}
export function planCandidateCompanionDispositionV1(inputs) {
    if (inputs.length !== 3)
        throw new Error("companion disposition requires exactly three rewrite groups");
    ...
    for (const input of inputs) {
        ...
        if (input.captureSafetyLane === "command_trace_rejection_review"
        ...
        if (input.knowledgeCoverage !== "covered_by_existing_truth"
