...
export function adjudicateCandidatePostRewriteReviewV1(rows, decisions) {
    ...
    for (const row of rows) {
        for (const digest of [
            ...
            if (!isDigest(digest))
                throw new Error("post-rewrite adjudication row digest is invalid");
        if (rowByHash.has(row.itemIdSha256))
            throw new Error("post-rewrite adjudication rows must be unique");
    ...
    if (decisionByHash.size !== rowByHash.size
    ...
    return {
        summary: {
            ...
            manualSemanticRows: (EXPECTED_TARGET_ROWS - EXPECTED_DUPLICATE_ROWS),
