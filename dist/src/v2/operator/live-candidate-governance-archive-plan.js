...
export function candidateGovernanceSourceLogicalDigestV1(db) {
...
function principalBindingDigest(row) {
    return hash(JSON.stringify({
...
function liveBinding(db, row, input) {
    return {
...
function validatePriorPlan(value) {
    ...
    if (hash(JSON.stringify(core)) !== value.planDigest)
        throw new Error("prior candidate adjudication digest is invalid");
...
export function validateAppendedCandidateArchiveDecisionControlV1(value) {
    if (value?.schemaVersion !== 1
...
function expectedExtendedSource(prior) {
    return {
        ...prior,
...
export function validateLiveCandidateGovernanceArchivePlanV1(value, expectedDigest) {
    if (value?.schemaVersion !== 1
...
export function assertCandidateGovernanceLiveBindingV1(db, planned, row) {
...
export function candidateGovernanceRowsByHashV1(db) {
    return new Map(candidateRows(db).map((row) => [hash(row.item_id), row]));
}
function validateCurrentPlanBindings(db, plan) {
    ...
    for (const planned of plan.rows) {
        ...
        if (!row)
            throw new Error("candidate governance archive target mapping is incomplete");
...
export function createLiveCandidateGovernanceArchivePlanV1(input) {
    ...
    const priorLoaded = privateJson(input.priorAdjudicationPath);
    const prior = validatePriorPlan(priorLoaded.value);
    const decisionLoaded = privateJson(input.appendedDecisionPath);
    ...
    try {
        ...
        return { ...partial, planDigest: hash(JSON.stringify(planCore(partial))) };
    }
    finally {
        db.close();
    }
...
export function loadPrivateCandidateGovernanceControlV1(path) {
