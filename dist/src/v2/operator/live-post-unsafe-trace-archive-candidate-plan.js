...
export function createLivePostUnsafeTraceArchiveCandidatePlanV1(input) {
    ...
    const baseline = privateJson(input.priorBaselinePath);
    validatePriorBaseline(baseline.value);
    const plan = privateJson(input.archivePlanPath);
    validateLiveCandidateUnsafeTraceDispositionPlanV1(plan.value, input.planDigest);
    const apply = privateJson(input.applyReceiptPath);
    validateApply(apply.value, plan.value, plan.sha256);
    const postcheck = privateJson(input.postcheckPath);
    ...
    if ([...removed, ...protectedRewrite].some((itemIdSha256) => priorByHash.get(itemIdSha256)?.disposition !== "hold_candidate")) {
    ...
    if (rows.length !== postcheck.value.source.candidateRows
