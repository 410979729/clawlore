...
import { planCandidateDurableRewriteProposalV1, } from "../application/candidate-durable-rewrite-proposal.js";
...
function validateAdjudicationControl(value) {
    const plan = value;
...
function validateRewritePayload(value, adjudication, adjudicationSha256) {
    const payload = value;
...
export function createLiveCandidateDurableRewriteProposalPlanV1(input) {
    ...
    try {
      ...
      WHERE i.lifecycle='candidate' ORDER BY i.item_id`).all();
        ...
        const proposal = planCandidateDurableRewriteProposalV1(rewriteRows, payload.specifications, candidates.map((row) => row.content));
