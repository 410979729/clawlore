...
async function removeSqliteFamily(path) {
    await Promise.all([
        rm(path, { force: true }),
...
export async function createAndVerifyLegacyLiveEncryptedSnapshotV2(input) {
    ...
    if (new Set(paths).size !== paths.length)
        throw new Error("snapshot workflow paths must be distinct");
    if (existsSync(input.archivePath))
        throw new Error("encrypted archive destination already exists");
    if (existsSync(input.receiptPath))
        throw new Error("snapshot receipt destination already exists");
    ...
    try {
        ...
        if (!sourceStableDuringBackup)
            throw new Error("live legacy truth changed during encrypted snapshot verification");
