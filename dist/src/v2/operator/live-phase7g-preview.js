...
import { buildPhase7GControlBundleV1, PHASE7G_LEGACY_SEARCH_FIELD_ALLOWLIST_V1, } from "../application/phase7g-rollout-controls.js";
...
function privateSnapshotReceipt(path) {
...
function verifyArchive(path, expectedSha256) {
    ...
    if (hash(readFileSync(path)) !== expectedSha256)
...
function candidateReviewRow(row) {
...
export async function createLivePhase7GPreviewV1(input) {
    ...
    if (before.memoryTruth.rowCount !== loaded.value.snapshot.memoryTruthRows
    ...
    if (!sourceUnchanged)
        throw new Error("live source truth changed during Phase 7G preview");
