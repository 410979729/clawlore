...
import { adjudicateCandidateDuplicateTracesV1, } from "../application/candidate-duplicate-trace-adjudication.js";
...
function appendOnlySourceExtensionRows(current, baseline) {
...
function validateCaptureSafetyControl(value) {
    const plan = value;
...
function validateDecisionControl(value, capturePlan, captureSha256) {
    const control = value;
    ...
    try {
      ...
      WHERE i.lifecycle='candidate' ORDER BY i.item_id`).all();
        ...
        for (const [itemHash, planned] of plannedByItemHash) {
            ...
            if (!live)
                throw new Error("duplicate-trace capture-safety target mapping is incomplete");
            assertLiveRowMatchesPlan(live, planned);
        }
        const adjudication = adjudicateCandidateDuplicateTracesV1(capturePlan.rows.filter((row) => row.lane === "exact_duplicate_operational_trace_review"), decisionControl.decisions);
