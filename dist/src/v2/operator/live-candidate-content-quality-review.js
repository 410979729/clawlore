...
import { assessCandidateContentQualityV1, validateSourceLineageReceiptV1, } from "../application/candidate-content-quality-review.js";
...
function sourceMatches(live, expected) {
    ...
    try {
      ...
      WHERE i.lifecycle='candidate' ORDER BY i.item_id`).all();
        if (candidates.length !== before.candidateRows)
            throw new Error("content review candidate mapping is incomplete");
        const targetRows = candidates.filter((row) => targetHashes.has(hash(row.item_id)));
        if (targetRows.length !== targetHashes.size)
            throw new Error("content review target mapping is incomplete");
        ...
        const corpusRows = db.prepare("SELECT content FROM memory_items ORDER BY item_id").all();
