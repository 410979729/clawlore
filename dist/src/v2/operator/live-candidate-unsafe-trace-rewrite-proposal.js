...
import { planCandidateUnsafeTraceRewriteProposalV1, } from "../application/candidate-unsafe-trace-rewrite-proposal.js";
...
function appendOnlyConvergedExtensionRows(current, baseline) {
...
function validatePayload(value, sha256, disposition, applySha256, postcheckSha256) {
...
function assertLiveTarget(row, planned) {
...
export function validateLiveCandidateUnsafeTraceRewriteProposalPlanV1(value, expectedDigest) {
...
function loadBoundControls(input) {
    const disposition = privateJson(input.dispositionPlanPath);
    validateLiveCandidateUnsafeTraceDispositionPlanV1(disposition.value);
    const apply = privateJson(input.archiveApplyReceiptPath);
    const postcheck = privateJson(input.archivePostcheckPath);
    ...
    const payload = privateJson(input.rewritePayloadPath);
...
function buildProposal(input) {
    ...
    try {
        ...
        const proposal = planCandidateUnsafeTraceRewriteProposalV1(input.controls.disposition.value.rewriteDesigns, input.controls.payload.value.specifications, candidates.map((row) => row.content));
...
export function createLiveCandidateUnsafeTraceRewriteProposalPlanV1(input) {
    ...
    const summary = { ...built.proposal.summary, liveBindingMismatches: 0 };
    const core = {
...
export function acceptLiveCandidateUnsafeTraceRewriteProposalV1(input) {
    const controls = loadBoundControls(input);
    const loadedPlan = privateJson(input.proposalPlanPath);
    const plan = validateLiveCandidateUnsafeTraceRewriteProposalPlanV1(loadedPlan.value);
    const built = buildProposal({ sourcePath: input.sourcePath, controls });
    const expectedSummary = { ...built.proposal.summary, liveBindingMismatches: 0 };
    if (plan.dispositionPlanDigest !== controls.disposition.value.planDigest
    ...
    for (const marker of ["legacy:", "revision:", "Command hints:", "Files:\n", "/home/", "/tmp/"]) {
        if (serializedPlan.includes(marker))
            throw new Error("unsafe trace rewrite proposal plan leaked raw trace material");
    }
    return {
        schemaVersion: 1,
