...
import { normalizeTruthIdentifier, normalizeTruthSemanticText, } from "../domain/truth-write-policy.js";
...
function requiredTables(db) {
    ...
    const rows = db.prepare(`SELECT name FROM sqlite_master WHERE type IN ('table','view') AND name IN (${required.map(() => "?").join(",")})`).all(...required);
    const found = new Set(rows.map((row) => String(row.name)));
    const missing = required.filter((name) => !found.has(name));
    if (missing.length > 0)
        throw new Error(`v2_runtime_schema_missing:${missing.join(",")}`);
}
/**
 ...
 */
export function mirrorRuntimeV1WriteToV2(sqlitePath, input) {
    const validation = validateMemoryAddress(input.address);
    if (!validation.valid)
        throw new Error("v2_runtime_address_invalid");
    ...
    try {
        ...
        requiredTables(db);
        const existing = db.prepare("SELECT content,address_json,lifecycle,verification FROM memory_items WHERE item_id=?").get(itemId);
        if (existing) {
        ...
        VALUES (?,?,1,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'active','user_confirmed',NULL,?,?)`)
                .run(itemId, revisionId, content, category, JSON.stringify(input.address), input.address.tenantId, input.address.principalId, input.address.agentId, input.address.visibility, input.address.retention, input.address.workspaceId ?? null, input.address.projectId ?? null, input.address.conversationId ?? null, input.address.threadId ?? null, input.address.customerId ?? null, input.address.taskId ?? null, now, now);
...
export function runtimeV2MirrorToolContext(sqlitePath, enabled) {
    return enabled
