...
function isPersistedJsonEqual(left, right) {
...
export class SqliteExperienceStoreV2 {
    ...
    saveScratch(scratch) {
        ...
        WHERE snapshot_id=? AND child_session_id=? AND status='active'`).run(scratch.scratchId, scratch.retention, scratch.lifecycle, JSON.stringify(scratch), scratch.createdAt, scratch.snapshotId, scratch.childSessionId);
    ...
    updateEpisode(episode, expected) {
        ...
        this.transaction(() => {
            ...
            if (!isDeepStrictEqual(withoutKeys(expected, ["parentVerification", "lifecycle", "verificationReason", "updatedAt"]), withoutKeys(episode, ["parentVerification", "lifecycle", "verificationReason", "updatedAt"])))
    ...
    updatePlaybook(playbook, expected) {
        ...
        this.transaction(() => {
            ...
            if (!isDeepStrictEqual(withoutKeys(expected, ["lifecycle", "operatorReviewed", "supersededBy", "updatedAt"]), withoutKeys(playbook, ["lifecycle", "operatorReviewed", "supersededBy", "updatedAt"])))
    ...
    supersedePlaybook(expectedPrevious, successor, event) {
        ...
        this.transaction(() => {
            ...
            if (!current || !isPersistedJsonEqual(current, expectedPrevious)) {
                ...
                const storedEvent = db.prepare("SELECT entity_type,entity_id,event_type,actor,reason,created_at FROM experience_events_v2 WHERE event_id=?").get(event.eventId);
        ...
        WHERE playbook_id=? AND version=? AND lifecycle=? AND operator_reviewed=? AND payload_json=?`).run(supersededPrevious.lifecycle, supersededPrevious.supersededBy, JSON.stringify(supersededPrevious), supersededPrevious.updatedAt, expectedPrevious.playbookId, expectedPrevious.version, expectedPrevious.lifecycle, expectedPrevious.operatorReviewed ? 1 : 0, JSON.stringify(expectedPrevious));
            if (Number(result.changes) !== 1) {
