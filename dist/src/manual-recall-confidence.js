import { rankManualRecallLexicalEvidence, } from "./manual-recall-lexical-evidence.js";
 ...
 */
export function filterConfidentManualRecall(candidates, config, context = {}) {
    ...
    if (context.query?.trim()) {
        ...
        const results = ranked.filter((evidence, index) => {
            ...
            const lexicalBandQualified = index === 0 || (topRankingScore - evidence.rankingScore <= 0.025
                && evidence.lexicalCoverage >= Math.max(policy.minimumLexicalScore, topLexicalCoverage * 0.9));
            if (lexicalQualified && lexicalBandQualified)
                return true;
            const semanticScore = Math.max(evidence.candidate.sources.vector?.score ?? 0, evidence.candidate.sources.reranked?.score ?? 0);
...
export function expandedManualRecallCandidateLimit(_requestedLimit) {
    return 20;
}
