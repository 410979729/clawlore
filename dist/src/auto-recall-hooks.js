...
import { clampInt, parsePositiveInt } from "./plugin-config.js";
    ...
    api.on("before_prompt_build", async (event, ctx) => {
        ...
        const recallWork = async () => {
            ...
            if (querySelection.truncated) {
                api.logger.info(`clawlore: auto-recall query truncated from ${querySelection.originalLength} to ${recallQuery.length} chars source=${querySelection.source}`);
            }
            const autoRecallMaxItems = Math.min(clampInt(config.autoRecallMaxItems ?? 3, 1, 20), clampInt(config.maxRecallPerTurn ?? 10, 1, 50));
            ...
            if (minRepeated > 0) {
                ...
                const filtered = rankedResults.filter((result) => {
                    ...
                    if (redundant) {
                        api.logger.debug?.(`clawlore: skipping redundant memory hash=${diagnosticHash(result.entry.id)} (last seen at turn ${lastTurn}, current turn ${currentTurn}, min ${minRepeated})`);
            ...
            const eligible = finalResults.filter((result) => {
                ...
                const governance = autoRecallGovernanceEligibility(metadata);
            ...
            const effectivePerItemMaxChars = (() => {
                if (recallMode === "summary")
                    return Math.min(perItemMaxChars, 80);
                if (!intent)
                    return perItemMaxChars;
                if (intent.depth === "l0")
                    return Math.min(perItemMaxChars, 80);
                if (intent.depth === "full")
                    return Math.min(perItemMaxChars * 3, 1000);
                return perItemMaxChars;
            })();
            ...
            if (selected.length === 0) {
                ...
                for (const candidate of candidates) {
                    if (!traceStatusById.has(candidate.id))
                        traceStatusById.set(candidate.id, { status: "budget_filtered", reason: "budget_exhausted" });
                }
                writeTrace({
                    decision: "skipped",
            ...
            for (const result of results) {
                ...
                if (selectedIds.has(id))
                    ...
                    traceStatusById.set(id, { status: "budget_filtered", reason: "not_selected_within_budget" });
            }
            api.logger.debug?.(`clawlore: auto-recall stats hits=${results.length}, dedupFiltered=${dedupFilteredCount}, stateFiltered=${stateFilteredCount}, suppressedFiltered=${suppressedFilteredCount}, crossScopeFiltered=${crossScopeFilteredCount}, preBudgetItems=${preBudgetItems}, preBudgetChars=${preBudgetChars}, postBudgetItems=${selected.length}, postBudgetChars=${usedChars}, maxItems=${autoRecallMaxItems}, maxChars=${autoRecallMaxChars}, perItemMaxChars=${perItemMaxChars}`);
            writeTrace({
                decision: "injected",
                reason: "selected",
                result_count: results.length,
        ...
        try {
            ...
            clearTimeout(timeoutId);
            if (error?.message === "retrieval aborted")
                return;
