...
function isAgentDeclaredInConfig(cfg, agentId) {
    ...
    try {
        ...
        return Array.isArray(list) && list.some((candidate) => candidate &&
 ...
 */
export function createReflectionCommandOrchestrator(options, dependencies) {
    const warnedInvalidAgentIds = new Set();
    const random = dependencies.random ?? Math.random;
    const resolveRunAgentId = (cfg, sourceAgentId) => {
        if (!options.configuredAgentId)
            return sourceAgentId;
        if (isAgentDeclaredInConfig(cfg, options.configuredAgentId))
            return options.configuredAgentId;
        if (!warnedInvalidAgentIds.has(options.configuredAgentId)) {
            dependencies.logger.warn(`memory-reflection: memoryReflection.agentId "${options.configuredAgentId}" not found in cfg.agents.list; ` +
                `fallback to runtime agent "${sourceAgentId}".`);
            warnedInvalidAgentIds.add(options.configuredAgentId);
        }
        return sourceAgentId;
    };
    return async function runMemoryReflection(event) {
        ...
        try {
            ...
            if (!cfg) {
                dependencies.logger.warn(`memory-reflection: command:${action} missing cfg in hook context; skip reflection`);
                return;
            }
            ...
            dependencies.logger.info(`memory-reflection: command:${action} hook start; ` +
            ...
            if (!currentSessionFile || currentSessionFile.includes(".reset.")) {
                ...
                dependencies.logger.info(`memory-reflection: command:${action} session recovery start ` +
                ...
                for (const sessionsDir of searchDirs) {
                    const recovered = await findPreviousReflectionSessionFile(sessionsDir, currentSessionFile, currentSessionId);
                    if (recovered) {
                        dependencies.logger.info(`memory-reflection: command:${action} recovered session file ${recovered} from ${sessionsDir}`);
            ...
            if (!currentSessionFile) {
                ...
                dependencies.logger.warn(`memory-reflection: command:${action} missing session file after recovery ` +
                    `session=${dependencies.diagnosticIdentifier(currentSessionId)}; dirCount=${searchDirs.length}`);
                return;
            }
            const conversation = await readSessionConversationWithResetFallback(currentSessionFile, options.messageCount);
            if (!conversation) {
                dependencies.logger.warn(`memory-reflection: command:${action} conversation empty/unusable ` +
                    `session=${dependencies.diagnosticIdentifier(currentSessionId)}; ` +
            ...
            const toolErrorSignals = sessionKey
                ...
                : [];
            dependencies.logger.info(`memory-reflection: command:${action} reflection generation start ` +
            ...
            dependencies.logger.info(`memory-reflection: command:${action} reflection generation done ` +
            ...
            for (const mapped of extractInjectableReflectionMappedMemoryItems(reflectionText)) {
                ...
                if (!safety.allowed) {
                    dependencies.logger.debug(`memory-reflection: skipped unsafe mapped memory reason=${safety.reason} ` +
                ...
                try {
                    ...
                    dependencies.logger.warn(`memory-reflection: mapped memory duplicate pre-check failed, continue store: ` +
            ...
            dependencies.logger.warn(`memory-reflection: hook failed: ${dependencies.diagnosticErrorSummary(error)}`);
        }
        finally {
            if (sessionKey)
                dependencies.clearReflectionErrorState(sessionKey);
            dependencies.pruneReflectionState();
        }
    };
}
