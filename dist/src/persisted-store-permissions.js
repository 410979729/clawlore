...
function inspectEntry(path, inspection) {
    ...
    for (const entry of readdirSync(path))
        inspectEntry(join(path, entry), inspection);
}
/** Inspect a persisted-store tree without following symlinks. */
export function inspectOwnerOnlyTree(path) {
...
export function inspectOwnerOnlySqliteFamily(path) {
    if (process.platform === "win32") {
        return { ownerOnly: null, files: 0, directories: 0, unsafeEntries: 0 };
    }
...
function tightenEntry(path) {
    ...
    if (info.isDirectory()) {
        ...
        for (const entry of readdirSync(path))
            tightenEntry(join(path, entry));
        return;
    }
    chmodSync(path, 0o600);
}
/** Tighten an exact persisted-store tree without traversing symlinks. */
export function tightenOwnerOnlyTree(path) {
    if (process.platform !== "win32")
        tightenEntry(path);
}
export function tightenOwnerOnlySqliteFamily(path) {
    if (process.platform === "win32")
        return;
    for (const candidate of [path, `${path}-wal`, `${path}-shm`]) {
        if (existsSync(candidate))
            tightenEntry(candidate);
    }
}
