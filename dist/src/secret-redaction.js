...
function looksLikePlaceholder(value) {
    ...
    if (!normalized)
        return true;
    if (EXACT_PLACEHOLDER_VALUES.has(lower))
        return true;
    if (/^\$\{[A-Za-z0-9_.:-]+\}$/.test(normalized))
        return true;
    if (/^\{\{\s*[A-Za-z0-9_.:-]+\s*\}\}$/.test(normalized))
        return true;
    if (/^\[REDACTED_[A-Z0-9_]+\]?$/i.test(normalized))
        return true;
    if (/^(?:[A-Za-z0-9_-]+[-_])?(?:\.{3}|\*{4,})$/.test(normalized))
        return true;
    if (/^(?:\[|<)?(?:redacted|placeholder|secret|token|password)(?:\]|>)?$/i.test(normalized))
        return true;
...
function secretValue(pattern, match) {
...
function visitJsonStrings(value, visit) {
...
function findDirectSecret(text) {
    ...
    for (const span of findStructuredSecretSpans(text)) {
        if (!looksLikePlaceholder(span.value))
            return { name: "structured-secret-assignment" };
    }
    return null;
}
export function findSecret(text) {
    ...
    let nested = null;
    if (parsed)
        visitJsonStrings(parsed, (value) => { nested ??= findDirectSecret(value); });
    return nested;
}
export function containsSecret(text) {
    return findSecret(text) !== null;
}
function redactDirectSecrets(text) {
    let redacted = text;
...
export function redactKnownSecrets(text) {
    const parsed = parseJsonContainer(text);
