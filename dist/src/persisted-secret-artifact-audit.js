...
async function inventoryRoot(rootPath) {
...
async function scanTextArtifact(entry) {
    ...
    try {
        ...
        if (entry.device !== before.dev.toString()
...
function isRecognizedEncryptedContainer(prefix, size) {
    if (prefix.subarray(0, CLAWLORE_ARCHIVE_MAGIC.length).equals(CLAWLORE_ARCHIVE_MAGIC)) {
        ...
        if (prefix.length < lengthOffset + 4)
            return false;
        ...
        if (headerLength <= 0
            ...
            return false;
        try {
            const header = JSON.parse(prefix.subarray(headerStart, headerEnd).toString("utf8"));
    ...
    if (asciiPrefix.startsWith("age-encryption.org/v1\n"))
        return true;
    if (asciiPrefix.startsWith("-----BEGIN PGP MESSAGE-----"))
        return true;
    return false;
}
async function fingerprintOpaqueArtifact(entry) {
    if (entry.size > MAX_ARTIFACT_FILE_BYTES) {
        throw new Error("CLAWLORE_PERSISTED_ARTIFACT_FILE_SIZE_LIMIT_EXCEEDED");
    }
    const handle = await open(entry.absolutePath, constants.O_RDONLY | constants.O_NOFOLLOW);
    try {
        ...
        for await (const chunk of handle.createReadStream({ autoClose: false })) {
            ...
            if (prefixBytes < MAX_ENCRYPTED_HEADER_BYTES + 64) {
                const prefixPart = bytes.subarray(0, Math.min(bytes.length, MAX_ENCRYPTED_HEADER_BYTES + 64 - prefixBytes));
        ...
        return {
            encrypted: isRecognizedEncryptedContainer(Buffer.concat(prefixChunks, prefixBytes), Number(before.size)),
            scannedBytes: Number(before.size),
            stateMaterial: [
                entry.fileRef,
                before.size.toString(),
 ...
 */
export async function auditPersistedSecretArtifactRoots(rootPaths) {
    ...
    const inventories = [...new Map(rawInventories.map((inventory) => [inventory.canonicalRoot, inventory])).values()];
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            for (const payload of scanned.payloads)
                flaggedPayloads.add(payload);
            if (scanned.finding) {
    ...
    const finalInventories = [...new Map((await Promise.all(uniqueRoots.map(inventoryRoot)))
            .map((inventory) => [inventory.canonicalRoot, inventory])).values()];
    const inventoryState = (items) => items
        ...
        .sort();
    if (JSON.stringify(inventoryState(finalInventories))
        !== JSON.stringify(inventoryState(inventories))) {
        throw new Error("CLAWLORE_PERSISTED_ARTIFACT_INVENTORY_CHANGED_DURING_AUDIT");
    }
    const permissions = inventories.map((inventory) => inspectOwnerOnlyTree(inventory.canonicalRoot));
