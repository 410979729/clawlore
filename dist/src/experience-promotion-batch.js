...
export function ensurePromotionBatchSchema(db) {
    db.exec(`
...
function compactPromotionSummary(promotion) {
    return {
...
function insertBatch(db, params) {
  ...
  `).run(params.batchId, params.scopeId ?? "", params.status, params.dryRun ? 1 : 0, params.reviewerNote, params.requestedBy, BACKUP_HINT, JSON.stringify(compactPromotionSummary(params.promotion)), now, params.status === "applied" ? now : null, JSON.stringify({ source: "experience_promotion_batch" }));
  ...
  `);
    for (const item of params.promotion.items) {
        insertItem.run(randomUUID(), params.batchId, item.action, item.episode_id ?? "", item.playbook_id ?? "", item.risk_level ?? "", item.status ?? "", item.reason ?? "", now);
    }
}
