...
import { writeJson, } from "./cli-runtime-policy.js";
export function registerPrincipalCommands(runtime) {
    ...
    principal
        ...
        try {
            ...
            if (result.principalHash)
                console.log(`Principal hash: ${result.principalHash}`);
            if (result.conversationHash)
                console.log(`Conversation hash: ${result.conversationHash}`);
        }
        catch (error) {
