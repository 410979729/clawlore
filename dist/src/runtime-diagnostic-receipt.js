...
function instanceIdentity(value) {
    const raw = record(value);
...
function parseRuntimeDiagnosticReceipt(value) {
    const raw = record(value);
    ...
    if (readinessExpiresAt !== undefined)
        isoTime(readinessExpiresAt, "readiness_expiry");
    return {
        schemaVersion: 2,
        generatedAt,
...
function linuxProcessStartToken(processId) {
    const bootId = readFileSync("/proc/sys/kernel/random/boot_id", "utf8").trim();
    const stat = readFileSync(`/proc/${processId}/stat`, "utf8");
    const closeParen = stat.lastIndexOf(")");
    if (closeParen < 0)
        throw new Error("runtime_process_stat_invalid");
    const fieldsAfterCommand = stat.slice(closeParen + 1).trim().split(/\s+/);
    const startTicks = fieldsAfterCommand[19];
    if (!bootId || !/^\d+$/.test(startTicks ?? ""))
        throw new Error("runtime_process_stat_invalid");
    return `linux:${bootId}:${startTicks}`;
}
function windowsProcessStartToken(processId) {
    ...
    if (!/^\d+$/.test(ticks))
        throw new Error("runtime_process_start_invalid");
    return `win32:${ticks}`;
}
function processStartToken(processId) {
    if (process.platform === "linux")
        return linuxProcessStartToken(processId);
    if (process.platform === "win32")
        return windowsProcessStartToken(processId);
    if (processId === process.pid)
        return SELF_PROCESS_FALLBACK_TOKEN;
...
export function probeRuntimeProcessIdentity(identity) {
    try {
...
export function resolveRuntimeDiagnosticFile(resolvedDbPath) {
...
export function renewRuntimeDiagnosticReceipt(receipt, now = () => new Date()) {
...
export function invalidateRuntimeDiagnosticReceipt(receipt, reason = "runtime_diagnostic_stopped", now = () => new Date()) {
    const current = parseRuntimeDiagnosticReceipt(receipt);
...
export async function writeRuntimeDiagnosticReceipt(file, receipt) {
    await writePrivateFileAtomic(file, `${JSON.stringify(receipt, null, 2)}\n`);
}
async function readRuntimeDiagnosticReceipt(file) {
...
export async function assessRuntimeDiagnostic(input) {
    ...
    if (receipt.runtime.requestedMode !== input.configuredMode)
        issues.push("runtime_diagnostic_mode_mismatch");
    if (input.configuredMode === "disabled") {
        if (receipt.runtime.status !== "disabled")
            issues.push("runtime_diagnostic_disabled_status_mismatch");
        if (receipt.runtime.registeredHookCount !== 0)
            issues.push("runtime_diagnostic_disabled_hooks_present");
    }
    else {
        if (Date.parse(receipt.generatedAt) > nowMs + 60_000)
            issues.push("runtime_diagnostic_created_in_future");
        if (Date.parse(receipt.lease.heartbeatAt) > nowMs + 60_000)
            issues.push("runtime_diagnostic_heartbeat_in_future");
        if (Date.parse(receipt.lease.expiresAt) <= nowMs)
            issues.push("runtime_diagnostic_lease_expired");
        const processStatus = (input.processIdentityProbe ?? probeRuntimeProcessIdentity)(receipt.instance);
        if (processStatus === "dead")
            issues.push("runtime_diagnostic_process_dead");
        else if (processStatus === "mismatch")
            issues.push("runtime_diagnostic_process_identity_mismatch");
        else if (processStatus === "unavailable")
            issues.push("runtime_diagnostic_process_identity_unavailable");
        if (receipt.configDigest !== input.configDigest)
            issues.push("runtime_diagnostic_config_digest_mismatch");
        ...
        if (!receipt.binding)
            issues.push("runtime_diagnostic_binding_missing");
        if (receipt.readiness.status !== "ready")
            issues.push("runtime_diagnostic_readiness_not_ready");
        if (!receipt.readiness.bindingVerified)
            issues.push("runtime_diagnostic_binding_unverified");
        if (receipt.readiness.errors.length > 0)
            issues.push(...receipt.readiness.errors);
        ...
        if (receipt.runtime.status !== "registered")
            issues.push("runtime_diagnostic_not_registered");
        if (receipt.runtime.registeredHookCount !== 1
            || receipt.runtime.registeredHooks[0] !== "message_received") {
            issues.push("runtime_diagnostic_hook_contract_mismatch");
        }
    }
    if (receipt.runtime.writeEnabled)
        issues.push("runtime_diagnostic_writes_enabled");
    if (receipt.runtime.promptMutationEnabled)
        issues.push("runtime_diagnostic_prompt_mutation_enabled");
    if (receipt.runtime.contextEngineRegistered)
        issues.push("runtime_diagnostic_context_engine_registered");
