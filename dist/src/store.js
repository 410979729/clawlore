...
class MemoryStoreRuntime {
    ...
    async listVectorIds() {
        ...
        const scan = await collectLanceRows(() => this.table.query().select(["id"]));
        const ids = scan.rows
        ...
        try {
            ...
            return batch.flatMap((entry, index) => {
                ...
                if (!Array.isArray(vector) || vector.length !== this.config.vectorDim) {
                    errors.push(`${diagnosticIdentifier(entry.id)}: embedding dimension mismatch (expected ${this.config.vectorDim}, got ${Array.isArray(vector) ? vector.length : "missing"})`);
                    return [];
                }
                return [{ ...entry, vector }];
            });
    ...
    async lexicalFallbackSearch(query, limit, scopeFilter, options) {
        ...
        const scan = await collectLanceRows(createSearchQuery);
    ...
    async list(scopeFilter, category, limit = 20, offset = 0) {
        ...
        const scan = await collectLanceRows(createListQuery);
    ...
    async stats(scopeFilter) {
        ...
        const scan = await scanLanceRows(createStatsQuery, (rows) => {
            for (const row of rows) {
                ...
                const lifecycle = effectiveMemoryLifecycle(parseSmartMetadata(row.metadata, {
                    text: row.text, category: category, timestamp: Number(row.timestamp),
                }));
                counts[lifecycle] += 1;
            }
        });
    ...
    assertCompleteLanceScan(truncated, operation) {
        if (!truncated)
            return;
        this.noteVectorScanBudgetExhausted();
    ...
    async getVectorScopeCounts() {
        ...
        if (!this.table)
            return {};
        const counts = {};
        const scan = await scanLanceRows(() => this.table.query().select(["id", "scope"]), (rows) => {
            for (const row of rows) {
