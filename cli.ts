...
import {
  inspectLegacyAuthorityMigration,
  migrateLegacySqlAuthority,
} from "./src/sql-authority-migration.js";
...
function stableRecordEntries(record: Record<string, number>): Array<[string, number]> {
...
function collectExperienceHealth(db: DatabaseSync | null): Record<string, unknown> {
  const required = [
    "task_episodes",
...
function collectNightlyDigestHealth(db: DatabaseSync | null): Record<string, unknown> {
  if (!db) {
  ...
  authority
    ...
    .action((options) => {
      try {
        ...
        if (inspection.status !== "valid" && inspection.status !== "legacy") process.exitCode = 1;
      } catch (error) {
  ...
  graph
    ...
    .action(async (options) => {
      try {
        ...
        const counts = (result.counts || ((result.before as Record<string, unknown> | undefined)?.counts) || {}) as Record<string, unknown>;
