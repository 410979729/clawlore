...
const runtimeSource = await readFile(new URL("../src/core-memory-runtime.ts", import.meta.url), "utf8");
const pluginConfigSource = await readFile(new URL("../src/plugin-config.ts", import.meta.url), "utf8");
...
test("MiniMax embedding.groupId is parsed, forwarded, and sent as GroupId", () => {
  assert.match(runtimeSource, /groupId:\s*config\.embedding\.groupId/);
  assert.match(pluginConfigSource, /groupId:\s*\n\s*typeof embedding\.groupId === "string"\s*\n\s*\? resolveConfigString\(embedding\.groupId\)/);
