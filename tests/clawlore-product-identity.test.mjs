...
test("identity compatibility is explicit and live release checks stay canonical", () => {
  ...
  assert.match(indexSource, /CLAWLORE_LEGACY_DEFAULTS\.dataDirectoryName/);
  assert.match(indexSource, /resolveDefaultOauthPathWithCompatibility/);
  assert.match(cliSource, /CLAWLORE_LEGACY_DEFAULTS\.oauthDirectoryName/);
