...
import {
  createReflectionCommandOrchestrator,
} from "./src/reflection-command-orchestrator.js";
import { createReflectionTextGenerator } from "./src/reflection-generation.js";
import type { ReflectionErrorSignal } from "./src/reflection-contracts.js";
import {
  readSessionConversationWithResetFallback,
  redactReflectionText as redactSecrets,
    ...
    if (config.sessionStrategy === "memoryReflection") {
      ...
      const runMemoryReflection = createReflectionCommandOrchestrator(
        ...
        },
        {
          ...
          resolveTargetScope: (sourceAgentId, access) =>
            access.defaultScope ??
            (isSystemBypassId(sourceAgentId)
              ? config.scopes?.default ?? "global"
              : scopeManager.getDefaultScope(sourceAgentId)),
          getToolErrorSignals: (sessionKey, maxEntries) =>
            (reflectionErrorStateBySession.get(sessionKey)?.entries ?? []).slice(-maxEntries),
