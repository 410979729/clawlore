...
interface NoisePrototypeReader {
  readonly initialized: boolean;
  isNoise(textVector: number[]): boolean;
}
...
export async function filterEmbeddingNoiseInputs(params: {
  texts: string[];
  noiseBank?: NoisePrototypeReader;
  ...
  const safeTexts = params.texts.flatMap((rawText) => {
    ...
    if (!safety.allowed) {
      params.debugLog(
        `clawlore: smart-extractor: embedding noise input skipped ` +
  ...
  if (!params.noiseBank?.initialized) return safeTexts;
  ...
  for (const text of safeTexts) {
    ...
    try {
      ...
      if (!vector?.length || !params.noiseBank.isNoise(vector)) {
        ...
        params.debugLog(
          `clawlore: smart-extractor: embedding noise filtered: ${diagnosticTextSummary(text)}`,
        );
      }
    } catch {
      result.push(text);
