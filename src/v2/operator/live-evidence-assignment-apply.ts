...
import {
  createLiveEvidenceAssignmentPlanV1,
  type EvidenceAssignmentPlanV1,
} from "./live-evidence-assignment-plan.js";
...
interface CandidateSourceRow extends CandidateStateRow {
  source_id: string;
  evidence_json: string;
}

export interface LiveEvidenceAssignmentReceiptV1 {
  schemaVersion: 1;
...
function privateJson<T>(path: string, maximumBytes = CONTROL_MAX_BYTES): { value: T; sha256: string } {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("rollout control JSON is invalid");
...
function stableStateDigest(row: CandidateStateRow): string {
...
function planCore(plan: EvidenceAssignmentPlanV1): Record<string, unknown> {
  return {
...
function loadPlan(path: string, rolloutId: string, digest: string): {
  value: EvidenceAssignmentPlanV1;
  sha256: string;
} {
  const loaded = privateJson<EvidenceAssignmentPlanV1>(path);
...
export async function executeLiveEvidenceAssignmentV1(input: {
...
}): Promise<LiveEvidenceAssignmentReceiptV1> {
  ...
  try {
    ...
    const sourceRows = db.prepare(`SELECT i.item_id,i.current_revision_id,i.address_json,i.lifecycle,i.verification,
      ...
      .all() as CandidateSourceRow[];
    const sourcesByItem = new Map<string, CandidateSourceRow[]>();
    for (const row of sourceRows) {
      const rows = sourcesByItem.get(row.item_id) ?? [];
      rows.push(row);
      sourcesByItem.set(row.item_id, rows);
    }
    if (sourcesByItem.size !== liveStates.length) throw new Error("candidate source coverage is incomplete");
    ...
    for (const state of liveStates) {
      ...
      if (sources.length !== 1) throw new Error("target candidate must have exactly one current source row");
      const source = sources[0];
      const existing = parseRecord(source.evidence_json);
      if ("registryResolvedEvidenceV1" in existing) throw new Error("target evidence assignment already exists");
      const decision = target.decision as AssignmentDecision;
      const evidenceKind = decision === "propose_private_principal_evidence_assignment"
        ? "direct-principal"
        : "conversation-boundary";
      if (evidenceKind === "direct-principal") directPrincipalRows += 1;
      ...
      if (Number(result.changes) !== 1) throw new Error("evidence assignment did not update exactly one source row");
  ...
  const pendingOutboxUnchanged = scalar(db,
    "SELECT COUNT(*) FROM projection_outbox WHERE processed_at IS NULL") === beforePendingOutbox;
