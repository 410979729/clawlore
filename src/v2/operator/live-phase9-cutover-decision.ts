...
import {
  decideClawLorePhase9CutoverV1,
  type ClawLorePhase9DecisionInputV1,
  type ClawLorePhase9NoCutoverDecisionV1,
} from "../application/phase9-cutover-decision.js";
import type { LiveCandidatePostRewriteAdjudicationPlanV1 } from
  "./live-candidate-post-rewrite-adjudication.js";
import type { LiveCandidateUnsafeTraceRewritePostcheckV1 } from
  "./live-candidate-unsafe-trace-rewrite-apply.js";
...
export interface LiveClawLorePhase9NoCutoverReceiptV1 {
  schemaVersion: 1;
  ...
  candidatePromotionPlanDigest: string;
  phase8gPlanSha256: string;
  phase8gPlanDigest: string;
  rewritePostcheckSha256: string;
  configSha256: string;
  source: SourceStateV1 & {
    candidateUnverifiedRows: number;
    activeInjectableRows: number;
    currentContentDivergenceRows: number;
    integrity: "ok";
    foreignKeyViolations: 0;
    unchangedDuringDecision: true;
  };
  runtime: ClawLorePhase9DecisionInputV1["runtime"];
  phase8g: ClawLorePhase9DecisionInputV1["phase8g"];
  decision: ClawLorePhase9NoCutoverDecisionV1;
  planDigest: string;
}
...
function privateJson<T>(path: string): { value: T; sha256: string } {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Phase 9 input JSON is invalid");
...
function validateBaseline(value: CandidateBaselineV1): void {
...
function phase8gCore(value: LiveCandidatePostRewriteAdjudicationPlanV1) {
  return {
...
function validatePhase8g(value: LiveCandidatePostRewriteAdjudicationPlanV1): void {
  if (
    value?.schemaVersion !== 1
...
function validateRewritePostcheck(value: LiveCandidateUnsafeTraceRewritePostcheckV1): void {
  if (
    value?.schemaVersion !== 1
...
export function createLiveClawLorePhase9NoCutoverReceiptV1(input: {
  ...
  phase8gPlanPath: string;
  rewritePostcheckPath: string;
  configPath: string;
  now?: () => Date;
}): LiveClawLorePhase9NoCutoverReceiptV1 {
  const baseline = privateJson<CandidateBaselineV1>(input.candidateBaselinePath);
  validateBaseline(baseline.value);
  const phase8g = privateJson<LiveCandidatePostRewriteAdjudicationPlanV1>(input.phase8gPlanPath);
  ...
  const config = privateJson<OpenClawConfigV1>(input.configPath);
  const plugin = config.value.plugins?.entries?.["scope-recall-openclaw"];
  ...
  try {
    ...
    if (
      !sameSource(before, baseline.value.source)
    ...
    const candidateUnverifiedRows = scalar(
      db,
      "SELECT COUNT(*) FROM memory_items WHERE lifecycle='candidate' AND verification='unverified'",
    );
    const activeInjectableRows = scalar(
      db,
      "SELECT COUNT(*) FROM memory_items WHERE lifecycle='active' AND verification IN ('operator_reviewed','user_confirmed','tool_verified')",
    );
    const currentContentDivergenceRows = scalar(
      db,
      "SELECT COUNT(*) FROM memory_items i JOIN memory_truth l ON i.item_id='legacy:' || l.id WHERE i.content<>l.text",
    );
    ...
    if (!sameSource(before, after)) throw new Error("live source changed during query-only Phase 9 decision");
    const runtime: ClawLorePhase9DecisionInputV1["runtime"] = {
    ...
    const phase8gSummary: ClawLorePhase9DecisionInputV1["phase8g"] = {
    ...
    const source = {
      ...
      unchangedDuringDecision: true as const,
    };
    const core = {
