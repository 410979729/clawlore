...
import type { CandidateUnsafeTraceRewriteSpecificationV1 } from
  "../application/candidate-unsafe-trace-rewrite-proposal.js";
import {
  validateLiveCandidateUnsafeTraceRewriteProposalPlanV1,
  type LiveCandidateUnsafeTraceRewriteProposalAcceptanceV1,
...
interface CandidateBaselineV1 {
  ...
  candidatePromotionPlan: {
    ...
    rows: Array<{ itemIdSha256: string; disposition: string; reasonCodes: string[] }>;
    planDigest: string;
  };
  decision: {
    eligibleRows: number;
...
export interface LiveCandidateUnsafeTraceRewriteApplyReceiptV1 {
  schemaVersion: 1;
...
export interface LiveCandidateUnsafeTraceRewritePostcheckV1 {
  schemaVersion: 1;
  ...
  protectedTargetStateDigest: string;
  database: { integrity: "ok"; foreignKeyViolations: 0 };
  runtime: LiveCandidateUnsafeTraceRewriteApplyReceiptV1["runtime"];
}
...
function comparableSource(value: SourceStateV1): SourceStateV1 {
  return {
    v1Rows: value.v1Rows,
    v2Rows: value.v2Rows,
...
function validatePayload(
  ...
  if (
    value.dispositionPlanDigest !== plan.dispositionPlanDigest
...
function validateAcceptance(
  value: LiveCandidateUnsafeTraceRewriteProposalAcceptanceV1,
  plan: LiveCandidateUnsafeTraceRewriteProposalPlanV1,
  planSha256: string,
  payload: UnsafeTraceRewritePayloadV1,
  payloadSha256: string,
): void {
  if (
    value?.schemaVersion !== 1
...
function validateBaseline(value: CandidateBaselineV1, expectedDigest: string): CandidateBaselineV1 {
...
function exactSpecifications(
  payload: UnsafeTraceRewritePayloadV1,
  plan: LiveCandidateUnsafeTraceRewriteProposalPlanV1,
): Map<string, CandidateUnsafeTraceRewriteSpecificationV1> {
  ...
  const parts = [
    ...
    digestQuery(db, `SELECT rel.* FROM memory_relations rel
      ...
      WHERE source_revision.item_id NOT IN (${placeholders})
        AND target_revision.item_id NOT IN (${placeholders}) ORDER BY rel.relation_id`,
...
function protectedTargetDigest(db: DatabaseSync, itemIds: string[]): string {
...
function runtimeBoundary(): LiveCandidateUnsafeTraceRewriteApplyReceiptV1["runtime"] {
  return {
    v1FallbackReads: true,
    existingCandidateLifecycleMutationEnabled: false,
    contextEngineEnabled: false,
...
export async function executeLiveCandidateUnsafeTraceRewriteV1(input: {
...
}): Promise<LiveCandidateUnsafeTraceRewriteApplyReceiptV1> {
  ...
  const loadedPlan = privateJson<LiveCandidateUnsafeTraceRewriteProposalPlanV1>(input.planPath);
  const plan = validateLiveCandidateUnsafeTraceRewriteProposalPlanV1(loadedPlan.value, input.planDigest);
  const loadedPayload = privateJson<UnsafeTraceRewritePayloadV1>(input.payloadPath);
  const payload = validatePayload(loadedPayload.value, loadedPayload.sha256, plan);
  const loadedAcceptance = privateJson<LiveCandidateUnsafeTraceRewriteProposalAcceptanceV1>(input.proposalAcceptancePath);
  ...
  try {
    ...
    for (const planned of plan.rows) {
      ...
      if (!live) throw new Error("unsafe trace rewrite target disappeared before transaction");
      assertTargetMatches(live, planned);
    }
    for (const planned of [...plan.rows].sort((left, right) => left.itemIdSha256.localeCompare(right.itemIdSha256))) {
      ...
      const superseded = db.prepare(
        "UPDATE memory_revisions SET lifecycle='superseded' WHERE revision_id=? AND lifecycle='candidate'",
      ...
      db.prepare(`INSERT INTO memory_revisions
        ...
        VALUES (?,?,?,?,?,?,?,?)`).run(
        revisionId, live.item_id, Number(live.revision_no) + 1, proposedContent,
        "candidate", "unverified", live.valid_until, appliedAt,
      );
      ...
      db.prepare(`INSERT INTO memory_events
        ...
        VALUES (?,?,?,?,?,?,?)`).run(
        randomUUID(), live.item_id, revisionId, "corrected", "operator:bounded-unsafe-trace-rewrite",
        input.rolloutId, appliedAt,
      );
      const current = db.prepare(`UPDATE memory_items SET current_revision_id=?,revision_no=?,content=?,updated_at=?
        ...
        revisionId, Number(live.revision_no) + 1, proposedContent, appliedAt, live.item_id,
      );
      if (Number(current.changes) !== 1) throw new Error("unsafe trace rewrite current item update failed closed");
      ...
      if (Number(fts.changes) !== 1) throw new Error("unsafe trace rewrite current FTS update failed closed");
    }
    db.prepare(`INSERT INTO clawlore_rollouts_v2
      ...
      legacyBefore.memoryTruth.logicalDigest, EXPECTED_TARGET_ROWS, appliedAt,
    );
    if (
      !sameSource(sourceState(db), beforeSource)
    ...
    const superseded = plan.rows.filter((planned) => {
      const live = transactionCandidates.get(planned.itemIdSha256)!;
      const row = db.prepare("SELECT lifecycle FROM memory_revisions WHERE revision_id=?").get(live.current_revision_id) as
    ...
    const rewritten = targetItemIds.map((itemId) => db.prepare(`SELECT i.lifecycle,i.verification,
      ...
      FROM memory_items i JOIN memory_revisions r ON r.revision_id=i.current_revision_id WHERE i.item_id=?`).get(itemId)) as
    ...
    if (integrity !== "ok" || foreignKeyViolations !== 0) throw new Error("unsafe trace rewrite database integrity failed");
    db.exec("COMMIT");
  } catch (error) {
  ...
  if (
    !sameSource(afterSource, beforeSource)
...
export function createLiveCandidateUnsafeTraceRewritePostcheckV1(input: {
  ...
  validateAcceptance(loadedAcceptance.value, plan, loadedPlan.sha256, payload, loadedPayload.sha256);
  const specifications = exactSpecifications(payload, plan);
  const loadedApply = privateJson<LiveCandidateUnsafeTraceRewriteApplyReceiptV1>(input.applyReceiptPath);
  validateApplyReceipt(
    loadedApply.value, plan, loadedPlan.sha256, payload, loadedPayload.sha256,
    loadedAcceptance.sha256, input.rolloutId,
  );
  ...
  try {
    ...
    const targetItemIds = plan.rows.map((planned) => {
      ...
      if (!row) throw new Error("unsafe trace rewrite postcheck target mapping is incomplete");
      return row.item_id;
    }).sort();
    if (
      nonTargetDigest(db, targetItemIds) !== loadedApply.value.protectedNonTargetDigest
    ...
    for (const planned of plan.rows) {
      ...
      unverifiedRows += 1;
      const revisions = db.prepare("SELECT revision_id,lifecycle FROM memory_revisions WHERE item_id=?").all(live.item_id) as
        Array<{ revision_id: string; lifecycle: string }>;
      ...
      const receipt = evidence.unsafeTraceRewriteReceiptV1 as Record<string, unknown> | undefined;
      if (
      ...
      supersedesRelationRows += scalar(db, `SELECT COUNT(*) FROM memory_relations
        WHERE from_revision_id=? AND to_revision_id=? AND relation_type='supersedes'`,
      live.current_revision_id, oldRevision.revision_id);
      correctedEventRows += scalar(db, `SELECT COUNT(*) FROM memory_events
        WHERE item_id=? AND revision_id=? AND event_type='corrected'
          AND actor='operator:bounded-unsafe-trace-rewrite' AND reason=?`,
      live.item_id, live.current_revision_id, input.rolloutId);
      currentFtsRows += scalar(db, "SELECT COUNT(*) FROM memory_fts_v2 WHERE item_id=? AND content=? AND category=?",
        live.item_id, proposedContent, live.category);
      const compatibility = db.prepare("SELECT content FROM memory_fts_compat_v2 WHERE item_id=?").get(live.item_id) as
    ...
    const rollout = scalar(db, `SELECT COUNT(*) FROM clawlore_rollouts_v2
      WHERE rollout_id=? AND plan_digest=? AND rows_applied=? AND v1_fallback_reads=1
        AND context_engine_enabled=0 AND final_recall_cutover_enabled=0`,
    input.rolloutId, plan.planDigest, EXPECTED_TARGET_ROWS);
    if (
      candidateRowsCount !== 32
      || unverifiedRows !== 32
      || supersededRevisionRows !== 32
      || validRewriteReceiptRows !== 32
      || supersedesRelationRows !== 32
