...
import { validateMemoryAddress, type MemoryAddressV2 } from "../domain/memory-address.js";
...
import {
  computeLegacyPrincipalTruthStateDigestV1,
  computeLivePrincipalScopePlanDigestV1,
  computePrincipalV2StateDigestV1,
  createLivePrincipalScopePlanV1,
  type LivePrincipalScopePlanV1,
  type PrincipalScopePlanRowV1,
} from "./live-principal-scope-plan.js";
...
export interface LivePrincipalScopeApplyReceiptV1 {
  schemaVersion: 1;
  phase: "clawlore-live-principal-scope-apply";
  migrationId: string;
  status: "truth_applied_vector_pending" | "complete";
  ...
  source: {
    memoryTruthRows: number;
    preMigrationLogicalDigest: string;
  ...
  mutation: {
    v1ScopeRowsChanged: number;
    lifecycleProjectionRowsChanged: number;
    v2AddressRowsChanged: number;
    aclRowsChanged: number;
    sourceEvidenceRowsChanged: number;
    auditEventsWritten: number;
    vectorRepairRowsPending: number;
...
function loadPlan(input: {
  ...
  migrationId: string;
  planDigest: string;
}): { value: LivePrincipalScopePlanV1; sha256: string } {
  const loaded = privateJson<LivePrincipalScopePlanV1>(input.path);
...
function loadFreshSnapshot(input: {
  ...
  plan: LivePrincipalScopePlanV1;
}): { value: SnapshotReceiptV1; sha256: string; archiveSha256: string } {
  const receipt = privateJson<SnapshotReceiptV1>(input.receiptPath, 256 * 1024);
...
function currentContentLifecycleDigest(db: DatabaseSync): string {
...
function receiptFromStored(input: {
  ...
  plan: LivePrincipalScopePlanV1;
  idempotentReplay: boolean;
}): LivePrincipalScopeApplyReceiptV1 {
...
export async function executeLivePrincipalScopeApplyV1(input: {
...
}): Promise<LivePrincipalScopeApplyReceiptV1> {
  ...
  if (
    plan.value.target.scope !== target.scope
  ...
  try {
    ...
    if (stored) {
      if (
        stored.plan_digest !== input.planDigest
      ...
      const integrity = String(Object.values(replayDb.prepare("PRAGMA integrity_check").get() as Record<string, unknown>)[0]);
      const foreignKeyViolations = (replayDb.prepare("PRAGMA foreign_key_check").all() as unknown[]).length;
  ...
  if (
    currentSnapshot.schemaDigest !== snapshot.value.snapshot.schemaDigest
  ...
  try {
    ...
    if (existingStored) {
      if (
        existingStored.plan_digest !== input.planDigest
    ...
    const plannedRows = new Map(
      plan.value.rows.filter((row) => row.principalAssignmentEligible)
        .map((row) => [row.legacyIdSha256, row]),
    );
    const truthRows = db.prepare(`SELECT id,text,category,scope,importance,timestamp,metadata,
      metadata_text,updated_at FROM memory_truth ORDER BY id`).all() as TruthRow[];
    const targets: Array<{ truth: TruthRow; planned: PrincipalScopePlanRowV1 }> = [];
    for (const truth of truthRows) {
      ...
      if (!principalAssignmentEligible) continue;
    ...
    db.prepare(`INSERT INTO clawlore_principal_scope_migrations
      ...
      plan.value.source.memoryTruthLogicalDigest,
      hash(input.sourceScope),
      hash(input.targetSessionKey),
      target.scope,
      target.principalHash,
      "truth_applied_vector_pending",
      appliedAt,
    );
    ...
    for (const { truth, planned } of targets) {
      ...
      const v2 = v2Statement.get(itemId) as V2Row | undefined;
      if (!v2) throw new Error("principal-scope target lost its V2 mirror");
      const acl = aclStatement.all(itemId) as Array<Record<string, unknown>>;
      const sources = sourceStatement.all(v2.current_revision_id) as Array<{
        source_id: string;
        evidence_json: string;
      }>;
      const v2Digest = computePrincipalV2StateDigestV1({
        v2: v2 as unknown as Record<string, unknown>,
        acl,
      ...
      const address = parseRecord(v2.address_json) as unknown as MemoryAddressV2;
      ...
      const nextAddress: MemoryAddressV2 = {
        ...address,
      ...
      if (!validation.valid) throw new Error("principal-scope target address is invalid");
      ...
      const itemUpdate = db.prepare(`UPDATE memory_items SET address_json=?,principal_id=?,
        visibility='private',updated_at=? WHERE item_id=? AND current_revision_id=?`).run(
        JSON.stringify(nextAddress), boundary.principalKey, appliedAt, itemId, v2.current_revision_id,
      );
      if (Number(itemUpdate.changes) !== 1) throw new Error("principal-scope V2 address update was not exact");
      ...
      if (Number(aclUpdate.changes) !== 1) throw new Error("principal-scope ACL update was not exact");
      ...
      if (Number(sourceUpdate.changes) !== 1) throw new Error("principal-scope source evidence update was not exact");
      db.prepare(`INSERT INTO memory_events
        (event_id,item_id,revision_id,event_type,actor,reason,created_at)
        VALUES (?,?,?,?,?,?,?)`).run(
        id(), itemId, v2.current_revision_id, "principal_scope_assigned",
        "operator:principal-scope-migration", "exact_private_session_provenance", appliedAt,
      );
      applied += 1;
    }
    ...
    if (
      applied !== plan.value.summary.principalAssignmentRows
      ...
      || scalar(db, `SELECT COUNT(*) FROM memory_items i
        ...
        WHERE m.migration_id=? AND i.principal_id=? AND i.visibility='private'`,
        input.migrationId, boundary.principalKey) !== applied
      || scalar(db, `SELECT COUNT(*) FROM memory_acl a
        JOIN clawlore_principal_scope_migration_items m ON m.item_id=a.item_id
        WHERE m.migration_id=? AND a.owner_principal_id=? AND a.visibility='private'`,
        input.migrationId, boundary.principalKey) !== applied
    ...
    const appliedStored = db.prepare("SELECT * FROM clawlore_principal_scope_migrations WHERE migration_id=?")
      .get(input.migrationId) as Record<string, unknown> | undefined;
    if (!appliedStored) throw new Error("principal-scope committed receipt is missing");
    db.close();
