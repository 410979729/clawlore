...
import type { CandidateDurableRewriteSpecificationV1 } from "../application/candidate-durable-rewrite-proposal.js";
import type { LiveCandidateDurableRewriteProposalPlanV1 } from "./live-candidate-durable-rewrite-proposal.js";
...
interface DurableRewritePayloadV1 {
  schemaVersion: 1;
...
interface DurableRewriteAcceptanceV1 {
  schemaVersion: 1;
  ...
  summary: LiveCandidateDurableRewriteProposalPlanV1["summary"];
  live: SourceStateV1;
...
export interface LiveCandidateDurableRewriteApplyReceiptV1 {
  schemaVersion: 1;
...
function appendOnlyExtension(current: SourceStateV1, baseline: SourceStateV1): number {
...
function validatePlan(value: unknown, expectedDigest: string): LiveCandidateDurableRewriteProposalPlanV1 {
  const plan = value as LiveCandidateDurableRewriteProposalPlanV1;
  if (
  ...
  if (hash(JSON.stringify(core)) !== plan.planDigest) throw new Error("durable rewrite proposal plan digest is invalid");
  return plan;
}
...
function validatePayload(
  value: unknown,
  sha256: string,
  plan: LiveCandidateDurableRewriteProposalPlanV1,
): DurableRewritePayloadV1 {
  const payload = value as DurableRewritePayloadV1;
  if (
  ...
  if (
    payload.adjudicationPlanDigest !== plan.adjudicationPlanDigest
...
function validateAcceptance(
  ...
  plan: LiveCandidateDurableRewriteProposalPlanV1,
  planSha256: string,
  payload: DurableRewritePayloadV1,
  payloadSha256: string,
): DurableRewriteAcceptanceV1 {
  const acceptance = value as DurableRewriteAcceptanceV1;
  if (
...
function validateBaseline(value: unknown, expectedDigest: string): CandidateBaselineV1 {
  const baseline = value as CandidateBaselineV1;
...
function assertTargetMatches(row: CandidateRowV1, planned: LiveCandidateDurableRewriteProposalPlanV1["rows"][number]): void {
...
function nonTargetDigest(db: DatabaseSync, representativeItemIds: string[]): string {
...
function protectedRepresentativeDigest(db: DatabaseSync, itemIds: string[]): string {
...
export async function executeLiveCandidateDurableRewriteV1(input: {
...
}): Promise<LiveCandidateDurableRewriteApplyReceiptV1> {
  if (!/^[a-z0-9][a-z0-9._:-]{7,127}$/i.test(input.rolloutId)) throw new Error("durable rewrite rollout id is invalid");
  ...
  const loadedPlan = privateJson<LiveCandidateDurableRewriteProposalPlanV1>(input.planPath);
  const plan = validatePlan(loadedPlan.value, input.planDigest);
  const loadedPayload = privateJson<DurableRewritePayloadV1>(input.payloadPath);
  const payload = validatePayload(loadedPayload.value, loadedPayload.sha256, plan);
  const loadedAcceptance = privateJson<DurableRewriteAcceptanceV1>(input.proposalAcceptancePath);
  ...
  try {
    ...
    if (!sameSource(sourceState(db), beforeSource)) throw new Error("live source drifted before durable rewrite transaction");
    ...
    for (const planned of plan.rows) {
      ...
      if (!live) throw new Error("durable rewrite target disappeared before transaction");
      assertTargetMatches(live, planned);
    }
    for (const planned of representativePlans.sort((left, right) => left.itemIdSha256.localeCompare(right.itemIdSha256))) {
      ...
      if (Number(superseded.changes) !== 1) throw new Error("durable rewrite current revision supersession failed closed");
      db.prepare(`INSERT INTO memory_revisions
        ...
        VALUES (?,?,?,?,?,?,?,?)`).run(
        revisionId, live.item_id, Number(live.revision_no) + 1, specification.proposedContent,
      ...
      db.prepare(`INSERT INTO memory_events
        ...
        VALUES (?,?,?,?,?,?,?)`).run(
        randomUUID(), live.item_id, revisionId, "corrected", "operator:bounded-durable-rewrite", input.rolloutId, appliedAt,
      );
      const current = db.prepare(`UPDATE memory_items SET current_revision_id=?,revision_no=?,content=?,updated_at=?
        ...
        revisionId, Number(live.revision_no) + 1, specification.proposedContent, appliedAt, live.item_id,
      );
      if (Number(current.changes) !== 1) throw new Error("durable rewrite current item update failed closed");
      ...
      if (Number(fts.changes) !== 1) throw new Error("durable rewrite current FTS update failed closed");
    }
    db.prepare(`INSERT INTO clawlore_rollouts_v2
      ...
      legacyBefore.memoryTruth.logicalDigest, 3, appliedAt,
    );
    if (
    ...
    if (superseded !== 3) throw new Error("durable rewrite historical supersession bookkeeping is incomplete");
    ...
    if (integrity !== "ok" || foreignKeyViolations !== 0) throw new Error("durable rewrite database integrity failed");
    db.exec("COMMIT");
  } catch (error) {
