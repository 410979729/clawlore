...
import {
  buildLiveCompatibilityBackfillPlanV1,
  projectLegacySearchMetadataV1,
  type LivePhase7GPreviewReceiptV1,
} from "./live-phase7g-preview.js";
...
function privateJson(path: string, maximumBytes: number): {
...
function loadPreview(path: string, rolloutId: string, planDigest: string, now: Date): {
  value: LivePhase7GPreviewReceiptV1;
  sha256: string;
} {
...
export async function executeLiveCompatibilityBackfillV1(input: {
  ...
  const preexisting = scalar(db,
    "SELECT COUNT(*) FROM sqlite_master WHERE type IN ('table','view') AND name='memory_fts_compat_v2'");
  if (preexisting !== 0) {
    db.close();
  ...
  if (
    livePlan.planDigest !== input.planDigest
  ...
  const columns = (db.prepare("PRAGMA table_info(memory_fts_compat_v2)").all() as Array<{ name: string }>)
    .map((row) => row.name);
  ...
  if (
    projectionRows !== livePlan.expectedProjectionRows
