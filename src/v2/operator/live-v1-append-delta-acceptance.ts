...
export interface LiveV1AppendDeltaAcceptanceV1 {
  schemaVersion: 1;
  ...
  runtime: LiveV1AppendDeltaApplyReceiptV1["runtime"];
}
...
export async function createLiveV1AppendDeltaAcceptanceV1(input: {
  ...
  now?: () => Date;
}): Promise<LiveV1AppendDeltaAcceptanceV1> {
  const loadedPlan = privateJson<LiveV1AppendDeltaPlanReceiptV1>(input.planPath);
  ...
  let result: LiveV1AppendDeltaAcceptanceV1;
  try {
    ...
    const deltaRows = db.prepare(`SELECT i.item_id,i.content,i.lifecycle,i.verification,i.address_json,
      ...
      ORDER BY i.item_id,s.source_id`).all() as Array<{
      item_id: string;
      content: string;
      lifecycle: string;
      verification: string;
    ...
    const lifecycle = Object.fromEntries((db.prepare(`SELECT lifecycle,COUNT(*) AS rows FROM memory_items
      GROUP BY lifecycle ORDER BY lifecycle`).all() as Array<{ lifecycle: string; rows: number }>)
