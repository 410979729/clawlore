...
import {
  ...
  type LiveCandidateCompanionDispositionAcceptanceV1,
...
export interface LiveCandidateCompanionArchiveReceiptV1 {
  schemaVersion: 1;
  ...
  planDigest: string;
  planSha256: string;
  dispositionAcceptanceSha256: string;
...
export interface LiveCandidateCompanionArchivePostcheckV1 {
  schemaVersion: 1;
  ...
  database: { integrity: "ok"; foreignKeyViolations: 0 };
  runtime: LiveCandidateCompanionArchiveReceiptV1["runtime"];
}
...
function assertCompanionMatches(
  row: CandidateRowV1,
  planned: LiveCandidateCompanionDispositionPlanV1["rows"][number],
): void {
...
function validateAcceptance(
  value: LiveCandidateCompanionDispositionAcceptanceV1,
...
export async function executeLiveCandidateCompanionArchiveV1(input: {
  sourcePath: string;
  planPath: string;
  dispositionAcceptancePath: string;
...
}): Promise<LiveCandidateCompanionArchiveReceiptV1> {
  if (!/^[a-z0-9][a-z0-9._:-]{7,127}$/i.test(input.rolloutId)) throw new Error("companion archive rollout id is invalid");
  ...
  const loadedPlan = privateJson<LiveCandidateCompanionDispositionPlanV1>(input.planPath);
  const plan = validateLiveCandidateCompanionDispositionPlanV1(loadedPlan.value, input.planDigest);
  const loadedAcceptance = privateJson<LiveCandidateCompanionDispositionAcceptanceV1>(input.dispositionAcceptancePath);
  ...
  try {
    ...
    for (const planned of plan.rows) {
      ...
      if (!live) throw new Error("companion archive target disappeared before transaction");
      assertCompanionMatches(live, planned);
      ...
      if (Number(superseded.changes) !== 1) throw new Error("companion archive current revision supersession failed closed");
      ...
      db.prepare(`INSERT INTO memory_events
        ...
        VALUES (?,?,?,?,?,?,?)`).run(
        randomUUID(), live.item_id, revisionId, "archived", "operator:bounded-companion-disposition", input.rolloutId, appliedAt,
      );
      ...
      if (Number(current.changes) !== 1) throw new Error("companion archive current item update failed closed");
    }
    ...
    if (archived.some((row) => {
      ...
      const receipt = evidence.companionDispositionReceiptV1 as Record<string, unknown> | undefined;
    ...
    if (supersededRows !== 3) throw new Error("companion archive historical supersession is incomplete");
    ...
    if (integrity !== "ok" || foreignKeyViolations !== 0) throw new Error("companion archive database integrity failed");
    db.exec("COMMIT");
  } catch (error) {
...
export function inspectLiveCandidateCompanionArchiveV1(input: {
  ...
  try {
    ...
    const allRows = db.prepare(`SELECT i.item_id,i.current_revision_id,i.content,i.category,i.lifecycle,i.verification,
      ...
      FROM memory_items i JOIN memory_revisions r ON r.revision_id=i.current_revision_id`).all() as Array<Record<string, unknown>>;
    ...
    for (const planned of plan.rows) {
      ...
      if (!companion || !representative) throw new Error("companion archive postcheck target mapping is incomplete");
      if (
        companion.lifecycle !== "archived"
      ...
      if (
        representative.lifecycle !== "candidate"
      ...
      const disposition = evidence.companionDispositionReceiptV1 as Record<string, unknown> | undefined;
      if (
        disposition?.rolloutId !== apply.rolloutId
      ...
      if (relation !== 1) throw new Error("companion archive postcheck supersedes relation is invalid");
      ...
      String(companion.item_id), String(companion.current_revision_id), apply.rolloutId);
      if (events !== 1) throw new Error("companion archive postcheck archived event is invalid");
      ...
      if (projections.some((count) => count !== 1)) throw new Error("companion archive postcheck projection binding is invalid");
      projectionBindingRows += 1;
    }
    const integrity = String(Object.values(db.prepare("PRAGMA integrity_check").get() as Record<string, unknown>)[0]);
    const foreignKeyViolations = (db.prepare("PRAGMA foreign_key_check").all() as unknown[]).length;
    if (integrity !== "ok" || foreignKeyViolations !== 0) throw new Error("companion archive postcheck database integrity failed");
    return {
      schemaVersion: 1,
      phase: "clawlore-candidate-companion-soft-archive-postcheck",
