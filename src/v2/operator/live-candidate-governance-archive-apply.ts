...
import {
  GOVERNANCE_ARCHIVE_TARGET_ROWS,
  assertCandidateGovernanceLiveBindingV1,
  candidateGovernanceRowsByHashV1,
  candidateGovernanceSourceLogicalDigestV1,
  loadPrivateCandidateGovernanceControlV1,
  validateLiveCandidateGovernanceArchivePlanV1,
  type LiveCandidateGovernanceArchiveAcceptanceV1,
  type LiveCandidateGovernanceArchivePlanV1,
...
export interface LiveCandidateGovernanceArchiveReceiptV1 {
  schemaVersion: 1;
  ...
  sourceBeforeLogicalDigest: string;
  sourceAfterLogicalDigest: string;
  archive: {
    targetRows: 112;
    candidateRowsArchived: 112;
    rowsChangedThisRun: number;
...
export interface LiveCandidateGovernanceArchivePostcheckV1 {
  schemaVersion: 1;
  ...
  database: { integrity: "ok"; foreignKeyViolations: 0 };
  runtime: LiveCandidateGovernanceArchiveReceiptV1["runtime"];
}
...
function privateJson<T>(path: string, maximumBytes = 5 * 1024 * 1024): { value: T; sha256: string } {
...
function validateAcceptance(
  value: LiveCandidateGovernanceArchiveAcceptanceV1,
  plan: LiveCandidateGovernanceArchivePlanV1,
  planSha256: string,
): void {
  if (
...
function runtimeState(): LiveCandidateGovernanceArchiveReceiptV1["runtime"] {
  return {
    v1FallbackReads: false,
    existingCandidateLifecycleMutationEnabled: false,
    contextEngineEnabled: false,
...
function allCurrentRows(db: DatabaseSync): Array<Record<string, unknown>> {
  return db.prepare(`SELECT i.*,r.lifecycle AS revision_lifecycle,r.verification AS revision_verification,
    ...
    ORDER BY i.item_id`).all() as Array<Record<string, unknown>>;
}

function principalDigest(row: Record<string, unknown>): string {
  return hash(JSON.stringify({
...
function verifyAppliedTargets(
  db: DatabaseSync,
  plan: LiveCandidateGovernanceArchivePlanV1,
  rolloutId: string,
): LiveCandidateGovernanceArchivePostcheckV1["targetBinding"] {
  ...
  for (const planned of plan.rows) {
    ...
    const receipt = evidence.governanceArchiveReceiptV1 as Record<string, unknown> | undefined;
    if (
    ...
    const relation = db.prepare(`SELECT to_revision_id FROM memory_relations
      WHERE from_revision_id=? AND relation_type='supersedes'`).all(String(row.current_revision_id)) as
      Array<{ to_revision_id: string }>;
    ...
    if (projections.some((count) => count !== 1)) throw new Error("governance archive projection binding is invalid");
  }
  return {
    archivedRows: 112,
    validGovernanceReceiptRows: 112,
...
function validateDatabase(db: DatabaseSync): { integrity: "ok"; foreignKeyViolations: 0 } {
  ...
  if (integrity !== "ok" || foreignKeyViolations !== 0) throw new Error("candidate governance archive database integrity failed");
...
function receipt(input: {
  ...
  sourceAfterLogicalDigest: string;
  rowsChangedThisRun: number;
}): LiveCandidateGovernanceArchiveReceiptV1 {
  return {
    schemaVersion: 1,
    phase: "clawlore-candidate-governance-soft-archive-live-apply",
...
export async function executeLiveCandidateGovernanceArchiveV1(input: {
...
}): Promise<LiveCandidateGovernanceArchiveReceiptV1> {
  if (!/^[a-z0-9][a-z0-9._:-]{7,127}$/i.test(input.rolloutId)) throw new Error("governance archive rollout id is invalid");
  ...
  if (!Number.isInteger(maximumAgeSeconds) || maximumAgeSeconds <= 0) throw new Error("maximum snapshot age must be positive");
  const loadedPlan = loadPrivateCandidateGovernanceControlV1<LiveCandidateGovernanceArchivePlanV1>(input.planPath);
  const plan = validateLiveCandidateGovernanceArchivePlanV1(loadedPlan.value, input.planDigest);
  const loadedAcceptance = loadPrivateCandidateGovernanceControlV1<LiveCandidateGovernanceArchiveAcceptanceV1>(input.acceptancePath);
  ...
  try {
    const existing = db.prepare("SELECT * FROM clawlore_rollouts_v2 WHERE rollout_id=?").get(input.rolloutId) as
    ...
    for (const planned of plan.rows) {
      ...
      if (!live) throw new Error("governance archive target mapping is incomplete");
    ...
    try {
      ...
      for (const planned of plan.rows) {
        ...
        if (!live) throw new Error("governance archive target disappeared before transaction");
        assertCandidateGovernanceLiveBindingV1(db, planned, live);
        ...
        if (Number(superseded.changes) !== 1) throw new Error("governance archive revision supersession failed closed");
        ...
        db.prepare(`INSERT INTO memory_events
          ...
          VALUES (?,?,?,?,?,?,?)`).run(
          randomUUID(), live.item_id, revisionId, "archived", "operator:bounded-governance-archive", input.rolloutId, appliedAt,
        );
        const current = db.prepare(`UPDATE memory_items SET current_revision_id=?,revision_no=?,lifecycle='archived',updated_at=?
          WHERE item_id=? AND current_revision_id=? AND lifecycle='candidate' AND verification='unverified'`).run(
          revisionId, Number(live.revision_no) + 1, appliedAt, live.item_id, live.current_revision_id,
        );
        if (Number(current.changes) !== 1) throw new Error("governance archive current item update failed closed");
      }
      db.prepare(`INSERT INTO clawlore_rollouts_v2
        ...
        legacyBefore.memoryTruth.logicalDigest, GOVERNANCE_ARCHIVE_TARGET_ROWS, appliedAt,
      );
...
export function inspectLiveCandidateGovernanceArchiveV1(input: {
  ...
  planDigest: string;
  now?: () => Date;
}): LiveCandidateGovernanceArchivePostcheckV1 {
  const loadedPlan = loadPrivateCandidateGovernanceControlV1<LiveCandidateGovernanceArchivePlanV1>(input.planPath);
  const plan = validateLiveCandidateGovernanceArchivePlanV1(loadedPlan.value, input.planDigest);
  const loadedApply = privateJson<LiveCandidateGovernanceArchiveReceiptV1>(input.applyReceiptPath);
