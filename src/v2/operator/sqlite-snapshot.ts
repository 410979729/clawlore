...
export interface SqliteSnapshotManifestV2 {
  schemaVersion: 1;
  createdAt: string;
  sha256: string;
  bytes: number;
  ...
  tableCounts: Record<(typeof TRUTH_TABLES)[number], number>;
}
...
export async function inspectSqliteSnapshotV2(path: string, createdAt = new Date().toISOString()): Promise<SqliteSnapshotManifestV2> {
  ...
  try {
    ...
    if (foreignKeyViolations !== 0) throw new Error(`snapshot foreign key check failed: ${foreignKeyViolations}`);
    const schema = db.prepare("SELECT MAX(version) AS version FROM clawlore_schema").get() as Record<string, unknown>;
    ...
    ])) as SqliteSnapshotManifestV2["tableCounts"];
...
export async function createOnlineSqliteBackupV2(sourcePath: string, destinationPath: string): Promise<void> {
  ...
  const { backup } = require("node:sqlite") as {
    backup: (source: DatabaseSync, destination: string, options?: object) => Promise<void>;
  };
...
export async function restoreVerifiedSqliteSnapshotV2(input: {
  snapshotPath: string;
  destinationPath: string;
  expected: SqliteSnapshotManifestV2;
  now?: () => Date;
}): Promise<SqliteSnapshotManifestV2> {
  const source = await inspectSqliteSnapshotV2(input.snapshotPath, input.expected.createdAt);
  if (source.sha256 !== input.expected.sha256) throw new Error("snapshot checksum mismatch");
  if (source.truthSchemaVersion !== input.expected.truthSchemaVersion) throw new Error("snapshot schema mismatch");
