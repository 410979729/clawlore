...
export type CandidateEvidenceRemediationLaneV1 =
...
export interface CandidateEvidenceRemediationPlanV1 {
  schemaVersion: 1;
...
function privateJson(path: string, maxBytes = CONTROL_MAX_BYTES): { value: unknown; sha256: string } {
...
function sessionReferences(metadata: Record<string, unknown>): string[] {
...
function registryKeyFor(index: RegistryIndex, reference: string): string | undefined {
...
function isDirectKey(value: string): boolean {
...
function isLegacyAgentAlias(value: string): boolean {
  const parts = value.split(":");
  return parts.length === 3 && parts[0] === "agent" && parts[1] === parts[2];
}

function derivedSystem(metadata: Record<string, unknown>): boolean {
...
function classification(row: CandidateRow, metadata: Record<string, unknown>): string {
...
function laneFor(row: CandidateRow, registry: RegistryIndex): {
  ...
  const matched = [...new Set(references.map((reference) => registryKeyFor(registry, reference)).filter(Boolean))] as string[];
  if (matched.length > 1) {
    return {
...
export function createLiveCandidateEvidenceRemediationPlanV1(input: {
...
}): CandidateEvidenceRemediationPlanV1 {
  ...
  try {
    ...
    if (sourceRows.length !== before.candidateRows) throw new Error("candidate mapping is incomplete");
