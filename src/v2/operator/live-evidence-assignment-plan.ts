...
import {
  createLiveCandidateEvidenceRemediationPlanV1,
  type CandidateEvidenceRemediationLaneV1,
  type CandidateEvidenceRemediationPlanV1,
} from "./live-candidate-evidence-remediation.js";

const require = createRequire(import.meta.url);
type DatabaseSync = any;
const CONTROL_MAX_BYTES = 2 * 1024 * 1024;

type EvidenceAssignmentDecisionV1 =
...
interface LoadedRemediationPreviewV1 {
  value: CandidateEvidenceRemediationPlanV1;
  sha256: string;
}

export interface EvidenceAssignmentPlanV1 {
  schemaVersion: 1;
  ...
  decisions: Record<EvidenceAssignmentDecisionV1, number>;
  rows: Array<{
    itemIdSha256: string;
    currentStateDigest: string;
    lane: CandidateEvidenceRemediationLaneV1;
    decision: EvidenceAssignmentDecisionV1;
    resolver?: "sessions_registry_exact_private_v1" | "sessions_registry_exact_conversation_v1";
...
function privateFile(path: string, maxBytes = CONTROL_MAX_BYTES): { bytes: Buffer; sha256: string } {
  if (process.platform === "win32") preparePrivateFileForRead(path);
  const info = statSync(path);
  if (!info.isFile() || (process.platform !== "win32" && (info.mode & 0o077) !== 0) || info.size <= 0 || info.size > maxBytes) {
...
function loadRemediationPreview(path: string): LoadedRemediationPreviewV1 {
  const loaded = privateFile(path);
  const value = JSON.parse(loaded.bytes.toString("utf8")) as CandidateEvidenceRemediationPlanV1;
  if (
...
function decisionFor(row: CandidateEvidenceRemediationPlanV1["rows"][number]): {
  decision: EvidenceAssignmentDecisionV1;
  resolver?: EvidenceAssignmentPlanV1["rows"][number]["resolver"];
} {
...
function assertSameRemediationPlan(
  expected: CandidateEvidenceRemediationPlanV1,
  ...
  if (
    expectedRows.size !== actual.rows.length
...
export function createLiveEvidenceAssignmentPlanV1(input: {
  ...
  ])) as Record<EvidenceAssignmentDecisionV1, number>;
