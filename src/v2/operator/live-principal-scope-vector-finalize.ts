import type { MemoryStorePorts } from "../../memory-store-ports.js";
import type { VectorScopeMetadataUpdaterPort } from "../../vector-scope-metadata-updater.js";

type DatabaseSync = any;

export interface PrincipalScopeVectorFinalizeReceiptV1 {
  schemaVersion: 1;
  ...
  rowsVerified: number;
  vectorScopeRowsChanged: number;
  vectorsReconciledThisRun: number;
  pendingRepairRows: 0;
  completedAt: string;
  database: {
    integrity: "ok";
...
export async function finalizeLivePrincipalScopeVectorsV1(input: {
  store: Pick<MemoryStorePorts,
    "getSqlTruthDb" | "getById" | "getVectorEntryById">;
  scopeUpdater: Pick<VectorScopeMetadataUpdaterPort, "getScope" | "updateScope">;
  migrationId: string;
  expectedPlanDigest: string;
  now?: () => Date;
}): Promise<PrincipalScopeVectorFinalizeReceiptV1> {
  const db = await input.store.getSqlTruthDb() as DatabaseSync | null;
  if (!db) throw new Error("principal-scope vector finalization requires SQL truth");
  const migration = db.prepare(`SELECT * FROM clawlore_principal_scope_migrations
    WHERE migration_id=?`).get(input.migrationId) as Record<string, unknown> | undefined;
  if (!migration) throw new Error("principal-scope migration receipt is missing");
  ...
  const items = db.prepare(`SELECT memory_id,original_scope
    ...
    WHERE migration_id=? ORDER BY memory_id`).all(input.migrationId) as Array<{
      memory_id: string;
  ...
  if (
    !Number.isInteger(vectorScopeRowsChanged)
  ...
  for (const item of items) {
    ...
    if (!before) throw new Error("principal-scope vector companion row is missing");
    if (before.scope !== targetScope) {
      if (
        before.scope !== item.original_scope
      ...
      const changed = await input.scopeUpdater.updateScope(
        item.memory_id,
        item.original_scope,
        targetScope,
      );
      if (!changed) throw new Error("principal-scope vector updater did not report the required change");
  ...
  try {
    ...
    if (status !== "complete") {
      ...
      if (Number(result.changes) !== 1) throw new Error("principal-scope completion receipt update was not exact");
    }
    const transactionIntegrity = String(Object.values(
      db.prepare("PRAGMA integrity_check").get() as Record<string, unknown>,
    )[0]);
    const transactionForeignKeyViolations = (db.prepare("PRAGMA foreign_key_check").all() as unknown[]).length;
