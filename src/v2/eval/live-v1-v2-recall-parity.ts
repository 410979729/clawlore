...
export interface RecallParityActorV1 {
  tenantId: string;
  principalId: string;
  agentId: string;
  workspaceId?: string;
...
export interface RecallParityQueryV1 {
  queryText: string;
  legacyScopes: string[];
  actor: RecallParityActorV1;
  limit?: number;
}

export interface RecallParityQueryResultV1 {
  querySha256: string;
  scopeSetSha256: string;
  limit: number;
  v1Discovered: number;
  v2Discovered: number;
  nativeTopKOverlap: number;
  commonLaneTopKOverlap: number;
  commonLaneRankAgreement: number;
  v1WouldExposeOutsideV2Policy: number;
  v2PolicyEligible: number;
  v2Injectable: number;
  v2ForbiddenScopeLeakage: number;
  v1TopIdDigests: string[];
  v2TopIdDigests: string[];
}

export interface LiveV1V2RecallParityReportV1 {
  schemaVersion: 1;
  ...
  corpus: {
    v1Rows: number;
    v2Rows: number;
    missingV2Rows: number;
    duplicateLegacyMappings: number;
    contentNormalizationOnlyRows: number;
    substantiveContentMismatches: number;
    governanceAuthorizedContentRewrites: number;
    unauthorizedSubstantiveContentMismatches: number;
    governanceArchivedV2Rows: number;
    policyComparableRows: number;
    categoryMismatches: number;
    v1FtsRows: number;
    v2FtsRows: number;
    vectorFallbackRows: number;
    invalidVectorFallbackRows: number;
    active: number;
    candidate: number;
    archived: number;
  };
  queries: RecallParityQueryResultV1[];
  aggregate: {
    queryCount: number;
    minimumNativeTopKOverlap: number;
    minimumCommonLaneTopKOverlap: number;
    minimumCommonLaneRankAgreement: number;
    v2ForbiddenScopeLeakage: number;
    v2InjectableResults: number;
  };
  decision: {
    shadowReadReady: boolean;
...
function lexicalScore(query: string, row: ComparableRow): number {
...
function commonRank(query: string, rows: ComparableRow[], limit: number): ComparableRow[] {
  return rows
...
function overlap(left: ComparableRow[], right: ComparableRow[]): number {
...
function rankAgreement(left: ComparableRow[], right: ComparableRow[]): number {
  if (left.length === 0 && right.length === 0) return 1;
...
function parseAddress(value: string): MemoryAddressV2 | undefined {
  try {
    const parsed = JSON.parse(value) as MemoryAddressV2;
 ...
 */
function governanceRewriteAuthorized(v1: ComparableRow, v2: ComparableRow): boolean {
  const evidence = parseRecord(v2.evidence);
  for (const key of ["unsafeTraceRewriteReceiptV1", "durableRewriteReceiptV1"] as const) {
...
function policyEligible(row: ComparableRow, actor: RecallParityActorV1): boolean {
  const address = row.address;
  if (!address || address.tenantId !== actor.tenantId || address.agentId !== actor.agentId) return false;
  if (address.workspaceId && address.workspaceId !== actor.workspaceId) return false;
  if (address.threadId && address.threadId !== actor.threadId) return false;
...
function injectable(row: ComparableRow, actor: RecallParityActorV1): boolean {
...
function v1Inactive(metadata: string | undefined): boolean {
  try {
...
function logicalDigest(db: DatabaseSync): string {
  const rows = db.prepare(`SELECT id,text,category,scope,metadata FROM memory_truth ORDER BY id`).all() as
    Array<Record<string, unknown>>;
  const v2 = db.prepare(`SELECT item_id,current_revision_id,content,category,address_json,lifecycle,verification
    FROM memory_items ORDER BY item_id`).all() as Array<Record<string, unknown>>;
  return hash(JSON.stringify({ rows, v2 }));
}

function assertTables(db: DatabaseSync): void {
  const names = new Set((db.prepare("SELECT name FROM sqlite_master WHERE type IN ('table','view')").all() as
    Array<{ name: string }>).map((row) => row.name));
  const missing = REQUIRED_TABLES.filter((table) => !names.has(table));
  if (missing.length > 0) throw new Error(`recall parity requires tables: ${missing.join(",")}`);
}

function corpusRows(db: DatabaseSync): { v1: ComparableRow[]; v2: ComparableRow[] } {
  const v1 = (db.prepare(`SELECT id,text,category,scope,timestamp,metadata FROM memory_truth ORDER BY id`).all() as
    Array<Record<string, unknown>>).map((row) => ({
  ...
  const v2 = (db.prepare(`SELECT s.external_id AS id,i.content AS text,i.category,i.address_json,
    ...
    ORDER BY s.external_id`).all() as Array<Record<string, unknown>>).map((row) => ({
      id: String(row.id), text: String(row.text), category: String(row.category),
...
function nativeV1(db: DatabaseSync, query: RecallParityQueryV1, limit: number): ComparableRow[] {
  const match = ftsQuery(query.queryText);
  if (!match || query.legacyScopes.length === 0) return [];
  ...
  return (db.prepare(`SELECT m.id,m.text,m.category,m.scope,m.timestamp,m.metadata
    ...
    .all(match, ...query.legacyScopes, limit) as Array<Record<string, unknown>>)
    .map((row) => ({ id: String(row.id), text: String(row.text), category: String(row.category),
      scope: String(row.scope), metadata: String(row.metadata || "{}"), observedAt: Number(row.timestamp || 0) }))
    .filter((row) => !v1Inactive(row.metadata));
}

function nativeV2(db: DatabaseSync, query: RecallParityQueryV1, limit: number): ComparableRow[] {
  const match = ftsQuery(query.queryText);
  if (!match || query.legacyScopes.length === 0) return [];
  const placeholders = query.legacyScopes.map(() => "?").join(",");
  return (db.prepare(`SELECT s.external_id AS id,i.content AS text,i.category,i.address_json,
    ...
    .all(match, ...query.legacyScopes, limit) as Array<Record<string, unknown>>).map((row) => ({
      id: String(row.id), text: String(row.text), category: String(row.category),
      address: parseAddress(String(row.address_json)), lifecycle: String(row.lifecycle),
...
export function inspectLiveV1V2RecallParityV1(input: {
  sqlitePath: string;
  queries: RecallParityQueryV1[];
}): LiveV1V2RecallParityReportV1 {
  if (input.queries.length === 0) throw new Error("at least one recall parity query is required");
  ...
  try {
    ...
    const authorizedRewriteIds = new Set<string>();
    for (const [id, v1] of v1ById) {
      const v2 = v2ById.get(id);
      if (!v2) continue;
      if (hash(v1.text) !== hash(v2.text)) {
        if (hash(v1.text.trim()) === hash(v2.text)) contentNormalizationOnlyRows += 1;
        else {
      ...
      if (v1.category !== v2.category) categoryMismatches += 1;
    }
    ...
    const queryResults = input.queries.map((query): RecallParityQueryResultV1 => {
      const limit = Math.min(20, Math.max(1, Math.floor(query.limit ?? 10)));
      if (!query.queryText.trim()) throw new Error("recall parity query cannot be blank");
    ...
    if (before !== after) throw new Error("read-only recall parity source changed during snapshot");
    const minimum = (field: keyof Pick<RecallParityQueryResultV1,
      "nativeTopKOverlap" | "commonLaneTopKOverlap" | "commonLaneRankAgreement">) =>
      Math.min(...queryResults.map((row) => row[field]));
    const aggregate = {
    ...
    if (corpus.active === 0) cutoverBlockers.push("no_active_v2_memory");
    if (aggregate.v2InjectableResults === 0) cutoverBlockers.push("no_injectable_v2_recall_evidence");
    ...
    const report: LiveV1V2RecallParityReportV1 = {
      schemaVersion: 1,
      phase: "clawlore-v1-v2-recall-parity",
      readOnly: true,
      emitsMemoryContent: false,
