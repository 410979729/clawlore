...
function exactIdentifier(value: unknown, label: string, maxLength = 512): string {
  ...
  if (normalized !== value) throw new Error(`${label} contains invalid boundary whitespace`);
  return normalized;
}

function exactSemantic(value: unknown, label: string, maxLength = 4_000): string {
  const normalized = normalizeTruthSemanticText(value, label, maxLength, { collapseWhitespace: false });
  if (normalized !== value) throw new Error(`${label} contains invalid boundary whitespace`);
  return normalized;
}

function identifierList(value: unknown, label: string, maxItems = 256, maxLength = 512): string[] {
  if (!Array.isArray(value) || value.length > maxItems) throw new Error(`${label} list exceeds the size limit`);
  const normalized = value.map((item) => exactIdentifier(item, label, maxLength));
  if (new Set(normalized).size !== normalized.length) throw new Error(`${label} list contains duplicates`);
  return normalized;
}

function semanticList(value: unknown, label: string, maxItems = 128, maxLength = 4_000): string[] {
  if (!Array.isArray(value) || value.length > maxItems) throw new Error(`${label} list exceeds the size limit`);
...
function address(value: unknown, label: string): void {
  const validation = validateMemoryAddress(value as never);
  if (!validation.valid) throw new Error(`${label} is invalid`);
  assertMemoryAddressIdentifiersSafe(value as never);
}
...
export function assertSubagentSnapshotSafeForPersistence(snapshot: SubagentSnapshotV2): void {
  ...
  if (snapshot.parentSessionId === snapshot.childSessionId) throw new Error("child session must differ from parent session");
  if (snapshot.mode !== "isolated" && snapshot.mode !== "fork") throw new Error("subagent mode is unsupported");
  if (snapshot.status !== "active" && snapshot.status !== "revoked") throw new Error("subagent status is unsupported");
  ...
  const memoryIds = snapshot.items.map((item) => {
    if (!item || typeof item !== "object") throw new Error("subagent context item is required");
    const memoryId = exactIdentifier(item.memoryId, "context memory id");
    if (!CONTEXT_SECTIONS.has(item.section)) throw new Error("context memory section is unsupported");
    exactSemantic(item.text, "context memory text", 16_000);
    address(item.address, "context memory address");
    if (!VERIFICATIONS.has(item.verification)) throw new Error("context memory verification is unsupported");
    if (item.readOnly !== true) throw new Error("subagent context memory must be read-only");
    return memoryId;
  });
  if (new Set(memoryIds).size !== memoryIds.length) throw new Error("context memory ids must be unique");
  timestamp(snapshot.createdAt, "snapshot createdAt");
}

export function assertChildScratchSafeForPersistence(scratch: ChildScratchV2): void {
  if (!scratch || typeof scratch !== "object") throw new Error("child scratch is required");
  ...
  if (scratch.lifecycle !== "candidate") throw new Error("child scratch must remain candidate");
  timestamp(scratch.createdAt, "scratch createdAt");
}

export function assertExperienceEpisodeSafeForPersistence(episode: ExperienceEpisodeV2): void {
  ...
  if (!EPISODE_OUTCOMES.has(episode.outcome)) throw new Error("episode outcome is unsupported");
  ...
  if (!EXPERIENCE_LIFECYCLES.has(episode.lifecycle)) throw new Error("episode lifecycle is unsupported");
  ...
  if (episode.verificationReason != null) exactSemantic(episode.verificationReason, "verification reason");
  timestamp(episode.createdAt, "episode createdAt");
  timestamp(episode.updatedAt, "episode updatedAt");
}

export function assertProceduralPlaybookSafeForPersistence(playbook: ProceduralPlaybookV2): void {
  ...
  if (!Number.isSafeInteger(playbook.version) || playbook.version < 1) throw new Error("playbook version is invalid");
  ...
  if (new Set(stepIds).size !== stepIds.length) throw new Error("playbook step ids must be unique");
  ...
  if (new Set(gateIds).size !== gateIds.length) throw new Error("playbook gate ids must be unique");
  ...
  if (!EXPERIENCE_LIFECYCLES.has(playbook.lifecycle)) throw new Error("playbook lifecycle is unsupported");
  if (typeof playbook.operatorReviewed !== "boolean") throw new Error("operator review flag must be boolean");
  if (playbook.predecessorId != null) exactIdentifier(playbook.predecessorId, "predecessor id");
  if (playbook.supersededBy != null) exactIdentifier(playbook.supersededBy, "superseded playbook id");
...
export function assertExperienceEventSafeForPersistence(event: ExperienceEventV2): void {
  if (!event || typeof event !== "object") throw new Error("experience event is required");
  exactIdentifier(event.eventId, "experience event id");
  if (!EVENT_ENTITY_TYPES.has(event.entityType)) throw new Error("experience event entity type is unsupported");
