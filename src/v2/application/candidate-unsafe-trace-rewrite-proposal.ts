...
import type {
  CandidateUnsafeTraceRewriteDesignRowV1,
  CandidateUnsafeTraceRewriteDesignV1,
} from "./candidate-unsafe-trace-disposition.js";

export type CandidateUnsafeTraceRewriteKnowledgeCoverageV1 =
  | "covered_by_existing_truth"
  | "materially_new_bounded_truth";

export interface CandidateUnsafeTraceRewriteSpecificationV1 {
  itemIdSha256: string;
  currentRevisionIdSha256: string;
  rewriteDesign: CandidateUnsafeTraceRewriteDesignV1;
  knowledgeCoverage: CandidateUnsafeTraceRewriteKnowledgeCoverageV1;
  knowledgeEvidenceDigest: string;
  proposedContents: string[];
}

export interface CandidateUnsafeTraceRewriteOutputV1 {
  ordinal: number;
...
export interface CandidateUnsafeTraceRewriteProposalRowV1 {
  itemIdSha256: string;
  currentRevisionIdSha256: string;
  ...
  category: string;
  captureSafetyPattern: CandidateUnsafeTraceRewriteDesignRowV1["captureSafetyPattern"];
  captureSafetyLane: CandidateUnsafeTraceRewriteDesignRowV1["captureSafetyLane"];
  reason: CandidateUnsafeTraceRewriteDesignRowV1["reason"];
  resultDigest: string;
  rewriteDesign: CandidateUnsafeTraceRewriteDesignV1;
  knowledgeCoverage: CandidateUnsafeTraceRewriteKnowledgeCoverageV1;
  knowledgeEvidenceDigest: string;
  outputs: CandidateUnsafeTraceRewriteOutputV1[];
  proposedOutputRows: number;
  maximumProposedRows: number;
...
export interface CandidateUnsafeTraceRewriteProposalV1 {
  summary: {
    targetRows: number;
    oversizedSegmentationRows: number;
    semanticExtractionRows: number;
    proposedDurableRows: number;
    captureSafeProposals: number;
    coveredByExistingTruthRows: number;
    materiallyNewTruthRows: number;
    corpusCollisionRows: 0;
    mutationReadyRows: 0;
  };
  rows: CandidateUnsafeTraceRewriteProposalRowV1[];
}
...
function validateSpecification(specification: CandidateUnsafeTraceRewriteSpecificationV1): void {
  for (const digest of [
    specification.itemIdSha256,
    specification.currentRevisionIdSha256,
    specification.knowledgeEvidenceDigest,
  ]) if (!isDigest(digest)) throw new Error("unsafe trace rewrite specification digest is invalid");
...
export function planCandidateUnsafeTraceRewriteProposalV1(
  rows: CandidateUnsafeTraceRewriteDesignRowV1[],
  specifications: CandidateUnsafeTraceRewriteSpecificationV1[],
  corpusContents: string[],
): CandidateUnsafeTraceRewriteProposalV1 {
  if (
    rows.length !== EXPECTED_REWRITE_ROWS
  ...
  for (const row of rows) {
    if (
      row.proposedAction !== "hold_for_separate_bounded_rewrite_proposal"
  ...
  const specificationsByItem = new Map<string, CandidateUnsafeTraceRewriteSpecificationV1>();
  ...
  const plannedRows: CandidateUnsafeTraceRewriteProposalRowV1[] = [];
  for (const row of [...rows].sort((left, right) => left.itemIdSha256.localeCompare(right.itemIdSha256))) {
    ...
    const outputs = specification.proposedContents.map((content, index): CandidateUnsafeTraceRewriteOutputV1 => {
      const safety = evaluateCaptureSafety(content);
      if (!safety.allowed) throw new Error("unsafe trace rewrite proposal still contains capture-unsafe material");
