...
function provenanceFailures(
  mode: ClawLoreRolloutModeV1,
  provenance: ReleaseReadinessProvenanceV1,
  now: Date,
): string[] {
  if (mode === "disabled") return [];
  const failures: string[] = [];
  if (!COMMIT_RE.test(provenance.sourceCommit)) failures.push("provenance_source_commit_invalid");
  for (const field of [
    "runtimeDigest",
    "packageDigest",
    "lockDigest",
    ...
    if (!SHA256_RE.test(provenance[field])) failures.push(`provenance_digest_invalid:${field}`);
  }
  if (!provenance.generatedBy.trim()) failures.push("provenance_generator_missing");
  const createdAt = Date.parse(provenance.createdAt);
  const expiresAt = Date.parse(provenance.expiresAt);
  if (!Number.isFinite(createdAt)) failures.push("provenance_created_at_invalid");
  if (!Number.isFinite(expiresAt)) failures.push("provenance_expires_at_invalid");
  if (Number.isFinite(createdAt) && createdAt > now.getTime() + 60_000) failures.push("provenance_created_in_future");
  if (Number.isFinite(expiresAt) && expiresAt <= now.getTime()) failures.push("provenance_expired");
  if (Number.isFinite(createdAt) && Number.isFinite(expiresAt) && expiresAt <= createdAt) {
    failures.push("provenance_expiry_order_invalid");
  }

  if (mode === "v2-write" || mode === "cutover") {
    if (provenance.lifecycle.active <= 0) failures.push("lifecycle_active_empty");
    if (provenance.shadow.sampleCount < 30) failures.push("shadow_sample_count_below_30");
    if (provenance.shadow.directSamples < 20) failures.push("shadow_direct_samples_below_20");
    if (provenance.shadow.groupSamples < 5) failures.push("shadow_group_samples_below_5");
    if (provenance.shadow.positiveCandidateSamples < 10) failures.push("shadow_positive_samples_below_10");
    if (provenance.shadow.overlapRatio < 0.8) failures.push("shadow_overlap_below_0_8");
    if (provenance.shadow.rankAgreement < 0.8) failures.push("shadow_rank_agreement_below_0_8");
    if (provenance.shadow.p95LatencyMs > 750) failures.push("shadow_latency_above_750ms");
    if (provenance.shadow.forbiddenViolations !== 0) failures.push("shadow_forbidden_violation");
    if (provenance.shadow.promptBudgetViolations !== 0) failures.push("shadow_prompt_budget_violation");
  }
  return failures;
}
