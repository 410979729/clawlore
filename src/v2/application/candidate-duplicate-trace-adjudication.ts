...
export type CandidateDuplicateTraceDecisionBasisV1 =
  | "covered_by_existing_truth"
  | "transient_operational_trace"
  | "durable_fact_requires_rewrite";

export type CandidateDuplicateTraceDispositionV1 =
  | "propose_soft_archive"
  | "hold_for_bounded_rewrite";

export interface CandidateDuplicateTraceGroupDecisionV1 {
  normalizedContentDigest: string;
  expectedGroupSize: number;
  disposition: CandidateDuplicateTraceDispositionV1;
  basis: CandidateDuplicateTraceDecisionBasisV1;
  evidenceDigest: string;
}

export interface CandidateDuplicateTraceAdjudicationRowV1 {
  itemIdSha256: string;
...
export interface CandidateDuplicateTraceAdjudicationV1 {
  summary: {
    targetGroups: number;
    targetRows: number;
    softArchiveGroups: number;
    softArchiveRows: number;
    rewriteHoldGroups: number;
    rewriteHoldRows: number;
    mutationReadyRows: 0;
  };
  groups: CandidateDuplicateTraceGroupDecisionV1[];
...
function validateDecision(decision: CandidateDuplicateTraceGroupDecisionV1): void {
...
export function adjudicateCandidateDuplicateTracesV1(
  ...
  const groups = new Map<string, CandidateCaptureSafetyReviewRowV1[]>();
  for (const row of duplicateRows) {
    if (
      row.captureSafetyReason !== "operational-trace"
    ...
    ]) if (!isDigest(digest)) throw new Error("duplicate-trace row digest is invalid");
    const group = groups.get(row.normalizedContentDigest) ?? [];
    group.push(row);
    groups.set(row.normalizedContentDigest, group);
  }
  for (const group of groups.values()) {
    if (group.length < 2) throw new Error("duplicate-trace group is not duplicated inside the exact lane");
  }

  const decisionsByDigest = new Map<string, CandidateDuplicateTraceGroupDecisionV1>();
  ...
  for (const digest of decisionsByDigest.keys()) {
    if (!groups.has(digest)) throw new Error("duplicate-trace decision targets an unknown group");
  }

  const orderedGroups = [...decisions].sort((left, right) =>
    left.normalizedContentDigest.localeCompare(right.normalizedContentDigest));
  const adjudicatedRows = duplicateRows.map((row): CandidateDuplicateTraceAdjudicationRowV1 => {
