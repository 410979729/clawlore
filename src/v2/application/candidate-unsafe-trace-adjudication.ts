...
import type { CandidateCaptureSafetyReviewRowV1 } from "./candidate-capture-safety-review.js";

export type CandidateUnsafeTraceDispositionV1 = "soft_archive_proposal" | "bounded_rewrite_hold";
export type CandidateUnsafeTraceReasonV1 =
...
export interface CandidateUnsafeTraceAdjudicationInputV1 {
  review: CandidateCaptureSafetyReviewRowV1;
  content: string;
}

export interface CandidateUnsafeTraceAdjudicationRowV1 {
  itemIdSha256: string;
  ...
  captureSafetyLane: CandidateCaptureSafetyReviewRowV1["lane"];
  oversized: boolean;
  resultDigest: string;
  resultLengthBand: "empty" | "le200" | "le1000" | "le4000" | "gt4000";
  disposition: CandidateUnsafeTraceDispositionV1;
  reason: CandidateUnsafeTraceReasonV1;
  proposedLifecycle: "candidate";
  proposedVerification: "unverified";
  mutationReady: false;
}

export interface CandidateUnsafeTraceAdjudicationV1 {
  counts: Record<CandidateUnsafeTraceDispositionV1, number>;
  reasons: Record<CandidateUnsafeTraceReasonV1, number>;
  summary: {
    targetRows: number;
...
function lengthBand(length: number): CandidateUnsafeTraceAdjudicationRowV1["resultLengthBand"] {
...
function reasonFor(input: CandidateUnsafeTraceAdjudicationInputV1, result: string): {
  disposition: CandidateUnsafeTraceDispositionV1;
  reason: CandidateUnsafeTraceReasonV1;
} {
  ...
  if (!result) return { disposition: "soft_archive_proposal", reason: "pure_operational_trace" };
  if (
    /^(?:我)?(?:先|现在|继续|马上|接下来)?(?:开始|正在|继续|准备|会|把|先把).{0,80}(?:排查|定位|读取|检查|修改|修复|验证|审计|同步|跑测试|收口)/u.test(result)
  ...
  if (
    /Phase\s*(?:[0-9]+[A-Z]?|[A-Z])|完整记录|运行报告|TODO、|迁移计划|项目交接|release gate|全量测试|提交[：:]|pack\s*\d+/iu.test(result)
  ) return { disposition: "soft_archive_proposal", reason: "project_report_covered_by_durable_artifact" };
  if (
    /最新稳定版|当前装的是|截至我刚查|盘口|赔率|比赛|GitHub Releases|npm\s+.*latest|缺\s*`?\d+`?\s*个提交/iu.test(result)
  ) return { disposition: "soft_archive_proposal", reason: "stale_external_snapshot" };
  if (
    /Gateway|healthz|NRestarts|PID\s*`?\d+|active\/running|当前会话|当前 live|刚实测|当前配置|当前状态/iu.test(result)
  ) return { disposition: "soft_archive_proposal", reason: "transient_runtime_state" };
  if (
    /已按授权处理完|归档到了|安装好了|装好了|已接上中断任务|已排好|不需要重启|可以直接列入|建好了/iu.test(result)
...
export function adjudicateCandidateUnsafeTracesV1(
  inputs: CandidateUnsafeTraceAdjudicationInputV1[],
): CandidateUnsafeTraceAdjudicationV1 {
  const seen = new Set<string>();
  const rows = inputs.map((input): CandidateUnsafeTraceAdjudicationRowV1 => {
    const review = input.review;
    if (seen.has(review.itemIdSha256)) throw new Error("unsafe trace adjudication rows must be unique");
  ...
  const dispositions: CandidateUnsafeTraceDispositionV1[] = ["soft_archive_proposal", "bounded_rewrite_hold"];
  const reasons: CandidateUnsafeTraceReasonV1[] = [
    "pure_operational_trace",
    "progress_or_smoke_noise",
    "project_report_covered_by_durable_artifact",
    "stale_external_snapshot",
  ...
  const counts = Object.fromEntries(dispositions.map((disposition) => [disposition,
    rows.filter((row) => row.disposition === disposition).length])) as Record<CandidateUnsafeTraceDispositionV1, number>;
  const reasonCounts = Object.fromEntries(reasons.map((reason) => [reason,
    rows.filter((row) => row.reason === reason).length])) as Record<CandidateUnsafeTraceReasonV1, number>;
  return {
    counts,
