import type {
  CandidateUnsafeTraceAdjudicationRowV1,
  CandidateUnsafeTraceReasonV1,
} from "./candidate-unsafe-trace-adjudication.js";

export type CandidateUnsafeTraceRewriteDesignV1 =
  | "segment_oversized_result"
  | "extract_durable_result";

export interface CandidateUnsafeTraceArchiveTargetV1 {
  itemIdSha256: string;
  ...
  category: string;
  captureSafetyPattern: CandidateUnsafeTraceAdjudicationRowV1["captureSafetyPattern"];
  captureSafetyLane: CandidateUnsafeTraceAdjudicationRowV1["captureSafetyLane"];
...
export interface CandidateUnsafeTraceRewriteDesignRowV1 {
  itemIdSha256: string;
  currentRevisionIdSha256: string;
  contentDigest: string;
  normalizedContentDigest: string;
...
export interface CandidateUnsafeTraceDispositionV1 {
  summary: {
    targetRows: number;
    softArchiveRows: number;
    boundedRewriteRows: number;
    oversizedSegmentationRows: number;
    semanticExtractionRows: number;
    mutationReadyRows: 0;
  };
  archiveRows: CandidateUnsafeTraceArchiveTargetV1[];
  rewriteDesigns: CandidateUnsafeTraceRewriteDesignRowV1[];
}

function common(row: CandidateUnsafeTraceAdjudicationRowV1) {
  return {
...
export function planCandidateUnsafeTraceDispositionV1(
  rows: CandidateUnsafeTraceAdjudicationRowV1[],
): CandidateUnsafeTraceDispositionV1 {
  if (rows.length === 0) throw new Error("unsafe trace disposition requires at least one row");
  ...
  const archiveRows = rows.filter((row) => row.disposition === "soft_archive_proposal")
    .map((row): CandidateUnsafeTraceArchiveTargetV1 => {
  ...
  const rewriteDesigns = rows.filter((row) => row.disposition === "bounded_rewrite_hold")
    .map((row): CandidateUnsafeTraceRewriteDesignRowV1 => {
