import type { ContextPackV1, ContextMemoryV1 } from "../domain/context-pack.js";
import type {
  ChildScratchV2,
  EpisodeOutcomeV2,
  ExperienceEpisodeV2,
...
export class SubagentExperienceServiceV2 {
  constructor(
    private readonly store: ExperienceStoreV2Port,
    private readonly clock: ExperienceClockV2,
  ) {}
  ...
  recordChildScratch(input: {
    ...
    childSessionId: string;
    content: string;
    retention: "ephemeral" | "working" | "durable";
  }): ChildScratchV2 {
  ...
  onSubagentEnded(input: {
    ...
    const episode: ExperienceEpisodeV2 = {
      schemaVersion: 2,
  ...
  createPlaybookCandidate(input: {
    ...
    if (taskClasses.size !== 1 || scopes.size !== 1) throw new Error("playbook evidence must share task class and scope");
    const steps = normalizeSteps(input.steps);
    const verificationGates = normalizeGates(input.verificationGates);
    const now = this.clock.now().toISOString();
    const playbook: ProceduralPlaybookV2 = {
      schemaVersion: 2,
  ...
  promotePlaybook(input: { playbookId: string; operatorReviewed?: boolean; actor: string; reason: string }): ProceduralPlaybookV2 {
    ...
    const updated: ProceduralPlaybookV2 = {
      ...playbook,
      lifecycle: "promoted",
  ...
  supersedePlaybook(input: {
    ...
    const verificationGates = normalizeGates(input.verificationGates);
    const now = this.clock.now().toISOString();
    const next: ProceduralPlaybookV2 = {
      ...previous,
      playbookId: this.clock.id("playbook"),
  ...
  evaluateReplay(input: {
    ...
    const result: ReplayEvaluationV2 = {
  ...
  private requireSnapshot(snapshotId: string, childSessionId: string): SubagentSnapshotV2 {
    const snapshot = this.store.getSnapshot(snapshotId);
    if (!snapshot || snapshot.status !== "active") throw new Error("active subagent snapshot not found");
    if (snapshot.childSessionId !== childSessionId) throw new Error("child session does not own snapshot");
    return snapshot;
  }

  private requireEpisode(episodeId: string): ExperienceEpisodeV2 {
    const episode = this.store.getEpisode(episodeId);
    if (!episode) throw new Error("experience episode not found");
    return episode;
  }

  private requirePlaybook(playbookId: string): ProceduralPlaybookV2 {
    const playbook = this.store.getPlaybook(playbookId);
    if (!playbook) throw new Error("procedural playbook not found");
    return playbook;
  }
