...
export interface OpenClawTranscriptReadOptions {
  dbPath: string;
  sessionId: string;
  scope: string;
  startMs?: number;
...
export interface OpenClawTranscriptSourceInspection {
  schemaVersion: 1;
...
export interface OpenClawTranscriptReadResult {
  chunks: DigestInputChunk[];
  inspection: OpenClawTranscriptSourceInspection;
}
...
function requiredColumns(db: DatabaseSync, table: string, expected: Set<string>): void {
...
function canonicalPrivateDatabasePath(dbPath: string): string {
  if (!dbPath.trim()) throw new Error("transcript database path is required");
  ...
  ] as const) enforcePrivateDatabaseFile(`${canonical}${suffix}`, label);
  return canonical;
}

function safeMessagePayload(rawEvent: unknown): SafeMessagePayload | null {
  ...
  if (typeof content === "string" && content.trim()) {
    ...
    for (const part of content) {
      ...
      if (SAFE_TEXT_TYPES.has(type)) {
        ...
        if (SAFE_TOOL_NAME.test(name)) toolNames.add(name);
      }
    }
  }
  ...
  return {
    role: role as SafeMessagePayload["role"],
    text,
    toolNames: [...toolNames].sort(),
  };
}

function transcriptProvenanceId(sessionId: string, seq: number): string {
...
function chunkText(payload: SafeMessagePayload): string {
 ...
 */
export function readOpenClawSqliteTranscript(
  options: OpenClawTranscriptReadOptions,
): OpenClawTranscriptReadResult {
  const canonical = canonicalPrivateDatabasePath(options.dbPath);
  const sessionId = requireDigestBoundaryIdentifier(
    options.sessionId,
  ...
  const maxEvents = Math.min(200, Math.max(
    1,
    boundedInteger(options.maxEvents, "transcript maxEvents", 25),
  ));
  const scanLimit = Math.min(4_000, maxEvents * 20);

  const { DatabaseSync: Sqlite } = require("node:sqlite") as {
  ...
  try {
    ...
    const rows = db.prepare(`
      ...
      LIMIT ?
    `).all(...parameters) as TranscriptRow[];

    const chunks: DigestInputChunk[] = [];
    for (const row of rows) {
      ...
      if (chunks.length >= maxEvents) break;
    }
    chunks.reverse();
    if (chunks.length === 0) {
