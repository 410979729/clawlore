...
export function validateStoragePath(dbPath: string): string {
  ...
  try {
    ...
    throw new Error(
      `CLAWLORE_STORAGE_PRIVATE_DIRECTORY_REQUIRED: path=${diagnosticIdentifier(resolvedPath)} parent=${diagnosticIdentifier(dirname(resolvedPath))} ${diagnosticErrorSummary(err)}`,
    );
  }
...
export class MemoryStore {
  ...
  private async doInitialize(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `Failed to create FTS index, falling back to vector-only search: ${diagnosticErrorSummary(err)}`,
      );
      this.ftsIndexCreated = false;
    }
  ...
  private async initializeSqlTruthStore(hasCompanionRows: boolean): Promise<void> {
    ...
    try {
      ...
      this.sqlTruthError = null;
      console.log(
        `clawlore: SQL truth companion ready (${truth.count()} rows, path=${diagnosticIdentifier(truth.path)})`,
      );
    } catch (err) {
  ...
  private syncSqlTruthUpsert(entry: MemoryEntry): void {
    if (!this.sqlTruthStore) return;
    try {
  ...
  private syncSqlTruthDelete(id: string): void {
    if (!this.sqlTruthStore) return;
    try {
  ...
  private writeSqlTruthUpsert(entry: MemoryEntry, operation: string): void {
  ...
  private writeSqlTruthDelete(id: string, operation: string): void {
    if (!this.sqlTruthStore) {
  ...
  private markVectorCompanionNeedsRepair(
    memoryId: string,
  ...
  private async addVectorCompanion(
    ...
    try {
      ...
      if (clearDebtOnSuccess) this.clearVectorCompanionRepair(entry.id);
      return true;
    } catch (err) {
  ...
  private async deleteVectorCompanionById(
    id: string,
    operation: string,
    clearDebtOnSuccess = true,
  ): Promise<boolean> {
    try {
      ...
      if (clearDebtOnSuccess) this.clearVectorCompanionRepair(id);
      return true;
    } catch (err) {
  ...
  private async replaceVectorCompanion(
    entry: MemoryEntry,
    ...
    if (added) {
      this.markVectorCompanionNeedsRepair(
        entry.id,
        "upsert",
        `${addOperation}-reconcile`,
        new Error("vector delete phase failed before a successful companion upsert"),
      );
    }
    return false;
  }

  private hydrateVectorResultsFromSql(
    results: MemorySearchResult[],
    scopeFilter: string[] | undefined,
    excludeInactive: boolean,
    limit: number,
  ): MemorySearchResult[] {
    if (!this.sqlTruthStore || results.length === 0) return results.slice(0, limit);
    const truthRows = this.sqlTruthStore.getByIds(
  ...
  private resolveSqlEntry(id: string, scopeFilter?: string[]): MemoryEntry | null {
  ...
  private async embedRebuildBatch(
  ...
  async hasId(id: string): Promise<boolean> {
    await this.ensureInitialized();
    if (this.sqlTruthStore) return this.sqlTruthStore.getById(id) !== null;
  ...
  async vectorSearch(vector: number[], limit = 5, minScore = 0.3, scopeFilter?: string[], options?: { excludeInactive?: boolean }): Promise<MemorySearchResult[]> {
    ...
    if (this.sqliteVectorStore) {
      if (this.sqlTruthStore) {
        ...
        while (true) {
          ...
          const hydrated = this.hydrateVectorResultsFromSql(
            sqliteResults,
            scopeFilter,
            inactiveFilter,
            safeLimit,
      ...
      const sqliteResults = this.sqliteVectorStore.search(
        vector,
        initialFetchLimit,
        minScore,
        scopeFilter,
    ...
    while (true) {
      ...
      for (const row of results) {
        ...
        if (!this.sqlTruthStore && mapped.length >= safeLimit) break;
      }

      if (!this.sqlTruthStore) return mapped;
  ...
  async update(
    ...
    if (this.sqlTruthStore) {
      return this.runWithFileLock(() => this.runSerializedUpdate(async () => {
        ...
        if (Array.isArray(updated.vector) && updated.vector.length === this.config.vectorDim) {
          await this.replaceVectorCompanion(
            updated,
            "update-delete-old-vector",
            "update-add-vector",
          );
        } else {
          this.markVectorCompanionNeedsRepair(
            ...
            "upsert",
            "update",
            new Error(`missing ${this.config.vectorDim}-dimension vector for ${updated.id}`),
          );
        }
    ...
    return this.runWithFileLock(() => this.runSerializedUpdate(async () => {
      ...
      try {
        ...
        try {
          await this.table!.add([rollbackCandidate as any]);
  ...
  async supersede(
    ...
    if (!Array.isArray(replacement.vector) || replacement.vector.length !== this.config.vectorDim) {
      throw new Error(
        `Vector dimension mismatch: expected ${this.config.vectorDim}, got ${Array.isArray(replacement.vector) ? replacement.vector.length : "non-array"}`,
      );
    }

    return this.runWithFileLock(() => this.runSerializedUpdate(async () => {
      const original = this.resolveSqlEntry(id, scopeFilter);
      if (!original) throw new Error(`Memory ${id} not found or outside accessible scopes`);
      ...
      if (!metadata.factKey.trim()) throw new Error("Atomic supersede requires a non-empty fact key");

      const invalidatedOld: MemoryEntry = {
        ...original,
      ...
      if (Array.isArray(invalidatedOld.vector) && invalidatedOld.vector.length === this.config.vectorDim) {
        await this.replaceVectorCompanion(
          invalidatedOld,
          "supersede-delete-old-vector",
          "supersede-add-invalidated-old-vector",
        );
      } else {
        this.markVectorCompanionNeedsRepair(
          invalidatedOld.id,
          "upsert",
          "supersede-old",
          new Error(`missing ${this.config.vectorDim}-dimension vector for ${invalidatedOld.id}`),
        );
      }
  ...
  getDiagnostics(): MemoryStoreDiagnostics {
    let sqlTruth: MemoryStoreDiagnostics["sqlTruth"];
    if (!this.sqlTruthStore) {
      sqlTruth = {
        available: false,
        path: null,
  ...
  async rebuildVectorCompanion(
    ...
    const repairUpsertIds = new Set(
      repairDebt.filter((entry) => entry.action === "upsert").map((entry) => entry.memoryId),
    );
    const repairDeleteIds = new Set(
      repairDebt.filter((entry) => entry.action === "delete").map((entry) => entry.memoryId),
    );
    ...
    return this.runWithFileLock(async () => {
      ...
      for (let i = 0; i < entries.length; i += batchSize) {
        ...
        for (const entry of rebuiltEntries) {
          if (await this.replaceVectorCompanion(
            entry,
            "repair-delete-old-vector",
            "repair-add-vector",
          )) {
            result.rebuilt++;
          } else {
