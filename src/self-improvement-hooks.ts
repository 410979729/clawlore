...
async function loadReminder(workspaceDir?: string): Promise<string> {
  const baseDir = typeof workspaceDir === "string" ? workspaceDir.trim() : "";
  if (!baseDir) return DEFAULT_REMINDER;
  try {
...
export function registerSelfImprovementHooks(params: {
  ...
  if (config.selfImprovement?.enabled !== true) return;
  ...
  api.registerHook("agent:bootstrap", async (event) => {
    try {
      ...
      const exists = bootstrapFiles.some((file) => {
        ...
        return (file as Record<string, unknown>).path === "SELF_IMPROVEMENT_REMINDER.md";
      });
      if (exists) return;
      bootstrapFiles.push({
  ...
  if (config.selfImprovement?.beforeResetNote !== false) {
    const appendNote = async (event: any) => {
      try {
        ...
        api.logger.info(
          `self-improvement: command:${action} hook start; session=${diagnosticIdentifier(sessionKey)}; source=${diagnosticIdentifier(commandSource)}; hasMessages=${Array.isArray(event?.messages)}; contextKeys=${contextKeys || "(none)"}`,
        );
        if (!Array.isArray(event.messages)) {
          api.logger.warn(`self-improvement: command:${action} missing event.messages array; skip note inject`);
          return;
        }
        if (event.messages.some((message: unknown) => typeof message === "string" && message.includes(NOTE_PREFIX))) {
