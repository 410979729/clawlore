...
export function reviewPlaybook(
  ...
  return withExperienceTransaction(db, () => {
    const playbook = getPlaybook(db, playbookId, scopeIds);
    if (!playbook) return { reviewed: false, id: playbookId, error: "not_found" };
    if (status === "superseded") {
      const successorError = validatePlaybookSuccessor(
        playbook, supersededBy, (id) => getPlaybook(db, id, scopeIds),
      );
      if (successorError) return { reviewed: false, id: playbookId, error: successorError };
    }
    const now = new Date().toISOString();
    const currentVersion = getLatestVersion(db, playbookId);
