 ...
 */
export function digestSourceIdentifier(value: unknown, prefix = "source"): string {
  ...
  try {
    ...
    if (ABSOLUTE_PATH.test(normalized)) throw new Error("absolute path provenance is not durable");
    return normalized;
  } catch {
...
export function requireDigestBoundaryIdentifier(value: unknown, label: string, fallback: string): string {
...
export function requireDigestSourceType(value: unknown, fallback: DigestSourceTypeBoundary): DigestSourceTypeBoundary {
  const sourceType = value ?? fallback;
  if (typeof sourceType !== "string" || !SOURCE_TYPES.has(sourceType as DigestSourceTypeBoundary)) {
...
export function digestLedgerRowForOutput<T extends Record<string, unknown>>(row: T): T {
  ...
  if ("source_id" in result) result.source_id = digestSourceIdentifier(result.source_id, "legacy-source");
  if ("scope" in result) result.scope = digestSourceIdentifier(result.scope, "legacy-scope");
  if ("actor" in result) result.actor = digestSourceIdentifier(result.actor, "legacy-actor");
  for (const field of ["reason", "preview", "notes", "candidate_ids"] as const) {
    if (field in result && typeof result[field] === "string") {
      result[field] = redactMemoryTextForOutput(result[field] as string);
    }
  }
  return result as T;
}

export function digestPreviewFor(value: string): string {
...
export function digestSafeJson(value: unknown): string {
...
export function parseDigestJsonObject(raw: unknown): Record<string, unknown> {
  if (!raw) return {};
