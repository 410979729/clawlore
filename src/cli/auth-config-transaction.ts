...
export function buildOauthLlmBackup(llm: unknown, hadLlmConfig: boolean): OAuthLlmBackup {
  return {
    version: 2,
    hadLlmConfig,
...
export type OAuthLoginConfigPlan = {
  hadLlmConfig: boolean;
  wasOauthMode: boolean;
  previousLlm: Record<string, unknown>;
  nextLlm: Record<string, unknown>;
};
...
export function planOAuthLoginConfig(options: {
  ...
  if (!wasOauthMode) delete nextLlm.baseURL;
  return {
    hadLlmConfig,
    wasOauthMode,
    previousLlm,
...
export type OAuthLoginBackupPlan = {
  writeBackup: boolean;
  hadLlmConfig: boolean;
...
export async function prepareOAuthLoginBackup(options: {
  ...
  loginPlan: OAuthLoginConfigPlan;
}): Promise<OAuthLoginBackupPlan> {
  ...
  const sourceOauthPath = resolveConfiguredOauthPath(
    options.configPath,
    options.loginPlan.previousLlm.oauthPath,
  );
...
function pluginIdentityContainers(config: Record<string, any>): {
  ...
  return { plugins, entries: plugins.entries as Record<string, any> };
}

function assertPluginEntry(value: unknown, id: string): asserts value is Record<string, any> {
  if (!isPlainObject(value)) {
...
function canonicalizePluginIdentityReferences(plugins: Record<string, any>): void {
  if (plugins.allow !== undefined) {
    if (!Array.isArray(plugins.allow) || plugins.allow.some((value: unknown) => typeof value !== "string")) {
      throw new Error("CLAWLORE_PLUGIN_IDENTITY_CONFIG_INVALID: plugins.allow must be a string array");
    }
    const canonicalAllow = plugins.allow.map((id: string) =>
      (CLAWLORE_LEGACY_PLUGIN_IDS as readonly string[]).includes(id) ? CLAWLORE_PLUGIN_ID : id
    );
  ...
  if (
    isPlainObject(plugins.slots) &&
    typeof plugins.slots.memory === "string" &&
    (CLAWLORE_LEGACY_PLUGIN_IDS as readonly string[]).includes(plugins.slots.memory)
  ) {
...
export function ensurePluginConfigRoot(config: Record<string, any>, pluginId: string): Record<string, any> {
  ...
  if (pluginId !== CLAWLORE_PLUGIN_ID) {
    ...
    return entries[pluginId].config as Record<string, any>;
  }

  const legacyIds = (CLAWLORE_LEGACY_PLUGIN_IDS as readonly string[]).filter((id) => entries[id] !== undefined);
  ...
  if (canonicalEntry !== undefined) assertPluginEntry(canonicalEntry, CLAWLORE_PLUGIN_ID);
  if (legacyEntry !== undefined) assertPluginEntry(legacyEntry, legacyId);
  ...
  if (plugins.allow !== undefined && (!Array.isArray(plugins.allow) || plugins.allow.some((value: unknown) => typeof value !== "string"))) {
    throw new Error("CLAWLORE_PLUGIN_IDENTITY_CONFIG_INVALID: plugins.allow must be a string array");
  }
  if (plugins.slots !== undefined && !isPlainObject(plugins.slots)) {
    throw new Error("CLAWLORE_PLUGIN_IDENTITY_CONFIG_INVALID: plugins.slots must be an object");
  ...
  if (legacyId) delete entries[legacyId];
...
export function getExistingPluginConfigRoot(config: Record<string, any>, pluginId: string): Record<string, unknown> {
  ...
  let entry = hasCanonicalEntry ? entries[pluginId] as Record<string, unknown> : {};
  if (pluginId === CLAWLORE_PLUGIN_ID) {
    const legacyEntries = (CLAWLORE_LEGACY_PLUGIN_IDS as readonly string[])
      .map((id) => entries[id])
...
export interface OpenClawConfigWriteHooks extends PrivateFileWriteOptions {
  beforeBackupWrite?: () => void | Promise<void>;
  afterBackupWrite?: () => void | Promise<void>;
  beforePostRenameValidation?: () => void | Promise<void>;
}

export function getOpenClawConfigBackupPath(configPath: string): string {
  return path.join(
    path.dirname(configPath),
    ".clawlore",
    "config-backups",
    `${path.basename(configPath)}.before-auth.json`,
  );
}
...
export async function saveOpenClawConfig(
  ...
  hooks: OpenClawConfigWriteHooks = {},
...
export interface OAuthLogoutTransactionHooks {
  configWrite?: OpenClawConfigWriteHooks;
  beforeOauthDelete?: () => void | Promise<void>;
  afterOauthDelete?: () => void | Promise<void>;
  beforeBackupDelete?: () => void | Promise<void>;
  afterBackupDelete?: () => void | Promise<void>;
}

async function syncRemovedFileParent(filePath: string): Promise<void> {
...
async function removeRegularFileDurably(filePath: string): Promise<void> {
  try {
...
export async function performOAuthLogoutConfigTransaction(options: {
  configPath: string;
  pluginId?: string;
  oauthPathOverride?: unknown;
  hooks?: OAuthLogoutTransactionHooks;
}): Promise<{
  oauthPath: string;
  oauthBackupPath: string;
  configBackupPath: string;
  previousAuth: "oauth" | "api-key";
}> {
  ...
  const { backupPath: configBackupPath } = await saveOpenClawConfig(
    options.configPath,
    openclawConfig,
    options.hooks?.configWrite,
  );
