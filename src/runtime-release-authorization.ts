...
import {
  ...
  recordRuntimeReleaseAuthorityV1,
  runtimeReleaseReadinessDigestV1,
  type RuntimeReleaseAuthorityInspectionV1,
  type RuntimeReleaseWriteModeV1,
} from "./runtime-release-authority.js";
import {
  loadRuntimeRolloutControlsV1,
  type RuntimeRolloutControlsV1,
} from "./adapters/openclaw/runtime-rollout-control.js";
import type { ReleaseArtifactBindingV1 } from "./v2/domain/release.js";

export interface RuntimeReleaseAuthorizationV1 extends RuntimeRolloutControlsV1 {
  authority: RuntimeReleaseAuthorityInspectionV1;
  authorityRecorded: boolean;
}

function authorityFailure(error: unknown): string {
  const message = error instanceof Error ? error.message : "";
  if (/^runtime_release_authority_[a-z0-9_:-]+$/i.test(message)) return message;
...
export function authorizeRuntimeReleaseV1(input: {
  ...
  if (
    ...
    if (
      fullControls.readiness?.status !== "ready"
