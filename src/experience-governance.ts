...
export interface ExperienceDebtItem {
  id: string;
  scope_id: string;
  task_class: string;
  title?: string;
...
export interface ExperienceDebtReport {
  status: "ok" | "attention_needed" | "schema_missing";
  generated_at: string;
  scope_id?: string;
  thresholds: {
    stale_candidate_days: number;
    failing_min_runs: number;
    failing_failure_multiplier: number;
  };
  totals: {
    episodes: Record<string, number>;
    playbooks: Record<string, number>;
    runs: Record<string, number>;
    capture_events: Record<string, number>;
    capture_skip_reasons: Record<string, number>;
  };
  debt: {
    ready_to_promote_episodes: { count: number; items: ExperienceDebtItem[] };
    blocked_success_episodes: { count: number; items: ExperienceDebtItem[] };
    review_backlog_playbooks: { count: number; items: ExperienceDebtItem[] };
    stale_candidate_playbooks: { count: number; items: ExperienceDebtItem[] };
    failing_playbooks: { count: number; items: ExperienceDebtItem[] };
    skipped_capture_events: { count: number; items: ExperienceDebtItem[] };
  };
...
function groupedCounts(rows: Array<Record<string, unknown>>, key: string): Record<string, number> {
...
function ageDays(value: unknown, nowMs: number): number {
...
function scopeWhere(scopeId?: string): { where: string; params: string[] } {
  if (!scopeId) return { where: "", params: [] };
  return { where: "WHERE (scope_id = ? OR shared_scope_id = ?)", params: [scopeId, scopeId] };
}

function runScopeWhere(scopeId?: string): { where: string; params: string[] } {
  if (!scopeId) return { where: "", params: [] };
...
function makeRecommendation(
  ...
  action: string,
  command?: string,
): { kind: string; count: number; command?: string; action: string } {
  return command ? { kind, count, command, action } : { kind, count, action };
}

export function buildExperienceDebtReport(
  db: DatabaseSync,
  options: ExperienceDebtOptions = {},
): ExperienceDebtReport {
  const nowMs = options.now_ms ?? Date.now();
  const generatedAt = new Date(nowMs).toISOString();
  const staleCandidateDays = Math.max(
    1,
    Number.parseInt(String(options.stale_candidate_days ?? DEFAULT_STALE_CANDIDATE_DAYS), 10)
      || DEFAULT_STALE_CANDIDATE_DAYS,
  );
  const limit = clampLimit(options.limit);
  const missing = missingTables(db);

  const base: Omit<ExperienceDebtReport, "status"> = {
  ...
  `).all(...episodeScope.params) as Array<Record<string, unknown>>;
  ...
  `).all(...playbookScope.params) as Array<Record<string, unknown>>;

  const runs = db.prepare(`
    SELECT id, playbook_id, scope_id, decision, outcome, started_at, finished_at
    FROM experience_runs
    ${runScope.where}
  `).all(...runScope.params) as Array<Record<string, unknown>>;
  ...
  `).all(...captureScope.params) as Array<Record<string, unknown>>;
  ...
  const readyToPromote: ExperienceDebtItem[] = [];
  const blockedSuccess: ExperienceDebtItem[] = [];

  for (const row of successfulCompleted) {
    if (linkedEpisodeIds.has(String(row.id))) continue;
    ...
    if (evidenceCount === 0) reasons.push("missing_evidence");
    if (toolCount === 0) reasons.push("missing_tool_names");
    if (verificationCount === 0) reasons.push("missing_verification");
    if (containsSecretLikeText(textForSafety)) reasons.push("secret_like_content");

    const item: ExperienceDebtItem = {
      id: String(row.id),
  ...
  const reviewBacklog: ExperienceDebtItem[] = [];
  const staleCandidates: ExperienceDebtItem[] = [];
  const failingPlaybooks: ExperienceDebtItem[] = [];
  const skippedCaptureEvents: ExperienceDebtItem[] = [];
  ...
  for (const row of captureEvents) {
    if (String(row.action ?? "") !== "skipped") continue;
  ...
  const recommendations: ExperienceDebtReport["recommendations"] = [];
  ...
  if (recommendations.length === 0) {
    recommendations.push(makeRecommendation(
      "no_action",
      0,
      "No Experience governance debt matched the current thresholds.",
    ));
  }
  ...
  return {
    ...
    totals: {
      ...
      capture_skip_reasons: groupedCounts(
        captureEvents.filter((row) => String(row.action ?? "") === "skipped"),
        "reason",
      ),
    },
    debt: {
