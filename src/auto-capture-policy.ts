...
export function shouldCapture(text: string): boolean {
  ...
  if (normalized.length < minLength || normalized.length > 500) return false;
  if (!evaluateCaptureSafety(normalized).allowed) return false;
...
export type AutoCaptureCategory = "preference" | "fact" | "decision" | "entity" | "other";

/** Classify a regex-lane capture without changing its lifecycle or scope. */
export function detectCategory(text: string): AutoCaptureCategory {
