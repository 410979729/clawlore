...
export interface JournalRecoveryCandidate {
...
export interface JournalRecoveryReport {
  ...
  reason_prefixes: string[];
  missing_tables?: string[];
  by_reason: Record<string, number>;
  by_scope: Record<string, number>;
  items: JournalRecoveryCandidate[];
}

export interface JournalReplayResult extends JournalRecoveryReport {
  dry_run: boolean;
  batch_id: string;
  scheduled: number;
  entry_ids: number[];
...
const REQUIRED_TABLES = ["journal_entries", "journal_rejections", "memory_journal_sources"] as const;
...
function counts(items: JournalRecoveryCandidate[], key: "reason" | "scope"): Record<string, number> {
...
function findReplayCandidates(
  db: DatabaseSync,
  options: { reasonPrefixes?: string[]; limit?: number } = {},
  ...
  const rows = db.prepare(`
    ...
    LIMIT ?
  `).all(...params, limit * 10) as Array<Record<string, unknown>>;

  const seen = new Set<number>();
  const items: JournalRecoveryCandidate[] = [];
...
export function scheduleReplay(
  ...
  const result: JournalReplayResult = {
    ...report,
    dry_run: dryRun,
    batch_id: batchId,
    scheduled: 0,
  ...
  if (report.status === "unsupported" || dryRun || report.items.length === 0) return result;
  ...
  try {
    ...
    for (const item of report.items) {
      const current = db.prepare(
        "SELECT id, processed_run_id, processed_at FROM journal_entries WHERE id = ?",
      ).get(item.journal_entry_id) as Record<string, unknown> | undefined;
      if (!current || String(current.processed_run_id || "") !== item.run_id) continue;
      ...
      `).run(
        `gov_${randomUUID()}`,
        item.scope,
        String(item.journal_entry_id),
        batchId,
        JSON.stringify(item),
        JSON.stringify({ journal_entry_id: item.journal_entry_id, processed_run_id: "", processed_at: null }),
        item.reason,
        options.actor || "journal-recovery",
