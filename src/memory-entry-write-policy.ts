...
export interface PersistenceMemoryEntry {
  id?: unknown;
  text: unknown;
  vector: unknown;
  category: unknown;
 ...
 */
export function assertMemoryEntrySafeForPersistence(entry: PersistenceMemoryEntry): void {
  if (!entry || typeof entry !== "object") throw new Error("memory entry is required");
  if (entry.id != null) requireBoundedText(entry.id, "memory id", 512);
