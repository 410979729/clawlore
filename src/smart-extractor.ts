...
export interface ExtractPersistOptions {
   ...
   */
  scopeFilter?: string[];
  /** Stable runtime identity/scope evidence to persist with created or updated memories. */
  runtimeMetadata?: RuntimeScopeMetadata;
}
...
export class SmartExtractor {
  ...
  async extractAndPersist(
    ...
    for (const candidate of survivingCandidates) {
      if (
        ...
        this.log(
          `clawlore: smart-extractor: skipped USER.md-exclusive [${candidate.category}] ` +
  ...
  private async extractCandidates(
    ...
    if (result.degraded === true) {
      throw new Error(
        `extract-candidates degraded: ${result.degradedReason || result.degraded_reason || "unknown"}`,
      );
    }
    if (!result.memories || !Array.isArray(result.memories)) {
      throw new Error(
        `extract-candidates returned unexpected shape keys=${Object.keys(result).join(",") || "(none)"}`,
      );
    }
    ...
    for (const raw of result.memories) {
      ...
      if (!category) {
        ...
        this.debugLog(
          `clawlore: smart-extractor: dropping candidate due to invalid category ` +
      ...
      const candidateSafety = evaluateCaptureSafety(
        [abstract, overview, content].filter(Boolean).join("\n"),
      );
      ...
      if (!abstract || abstract.length < 5) {
        shortAbstractCount++;
        this.debugLog(
          `clawlore: smart-extractor: dropping candidate due to short abstract ` +
      ...
      if (isNoise(abstract)) {
        noiseAbstractCount++;
        this.debugLog(
          `clawlore: smart-extractor: dropping candidate due to noise abstract ` +
          `category=${category} abstract=${diagnosticTextSummary(abstract)}`,
        );
        continue;
      }
  ...
  private async processCandidate(
    ...
    if (admission?.decision === "reject") {
      stats.rejected = (stats.rejected ?? 0) + 1;
      this.log(
        `clawlore: smart-extractor: admission rejected [${candidate.category}] ` +
    ...
    switch (dedupResult.decision) {
      ...
      case "merge":
        if (
          ...
          const mergeResult = await this.handleMerge(
            candidate,
            dedupResult.matchId,
            vector,
            targetScope,
      ...
      case "skip":
        this.log(
          `clawlore: smart-extractor: skipped [${candidate.category}] ` +
          `abstract=${diagnosticTextSummary(candidate.abstract)}`,
        );
        stats.skipped++;
        break;
  ...
  private async deduplicate(
    ...
    const activeSimilar = filterUnsafeMemoryResults(await this.store.vectorSearch(
      candidateVector,
      5,
  ...
  private async llmDedupDecision(
    ...
    try {
      ...
      return {
        ...
        contextLabel: normalizeProviderAnnotation((data as any).context_label, 80),
      };
    } catch (err) {
      this.log(
  ...
  private async handleProfileMerge(
    ...
    if (!admissionAudit && this.admissionController && vector && vector.length > 0) {
      ...
      if (profileAdmission.decision === "reject") {
        this.log(
          `clawlore: smart-extractor: admission rejected profile ` +
    ...
    const existing = filterUnsafeMemoryResults(await this.store.vectorSearch(
      vector || [],
      1,
      0.3,
      scopeFilter,
  ...
  private async handleMerge(
    ...
    const storeFallback = async (reason: string): Promise<"created"> => {
    ...
    this.log(
      `clawlore: smart-extractor: merged [${candidate.category}] ` +
  ...
  private async handleSupersede(
    ...
    this.log(
      `clawlore: smart-extractor: superseded [${candidate.category}] ` +
  ...
  private async handleSupport(
    ...
    await this.store.update(
      matchId,
      { metadata: stringifySmartMetadata(this.withAdmissionAudit({ ...meta, ...runtimeMetadata }, admissionAudit)) },
      scopeFilter,
    );

    this.log(
      `clawlore: smart-extractor: support context=${diagnosticTextSummary(contextLabel || "general")} ` +
  ...
  private async handleContextualize(
    ...
    this.log(
      `clawlore: smart-extractor: contextualize context=${diagnosticTextSummary(contextLabel || "general")} ` +
  ...
  private async handleContradict(
    ...
    this.log(
      `clawlore: smart-extractor: contradict context=${diagnosticTextSummary(contextLabel || "general")} ` +
  ...
  private async storeCandidate(
    ...
    this.log(
      `clawlore: smart-extractor: created [${candidate.category}] ` +
      `abstract=${diagnosticTextSummary(candidate.abstract)}`,
    );
  }
