...
function legacyRepairOutboxStatus(db: DatabaseSync): "absent" | "compatible" | "incompatible" {
  const table = db.prepare(
    "SELECT type, sql FROM sqlite_master WHERE name = 'vector_companion_repair_outbox'",
  ...
  const index = db.prepare(
    "SELECT type, sql FROM sqlite_master WHERE name = 'idx_vector_companion_repair_updated_at'",
  ).get() as { type?: string; sql?: string | null } | undefined;
  if (
    index &&
  ...
  const unexpectedTrigger = db.prepare(
    "SELECT 1 FROM sqlite_master WHERE type = 'trigger' AND tbl_name = 'vector_companion_repair_outbox' LIMIT 1",
  ).get();
...
function actualSchemaFingerprint(db: DatabaseSync): string | null {
  ...
  const unexpected = db.prepare(`
    ...
    LIMIT 1
  `).get(...names, ...PROTECTED_SCHEMA_TABLES, ...PROTECTED_SCHEMA_TABLES) as {
    name?: string;
  } | undefined;
  if (unexpected?.name) return null;
...
function legacySnapshotDigestFromDb(db: DatabaseSync): string {
  ...
  if (repairOutboxStatus === "compatible") {
    for (const row of db.prepare(`SELECT memory_id,action,operation,last_error,attempts,created_at,updated_at
      FROM vector_companion_repair_outbox ORDER BY memory_id`).iterate() as Iterable<Record<string, unknown>>) {
      hash.update(`${JSON.stringify(row)}\n`);
    }
  }
  return hash.digest("hex");
