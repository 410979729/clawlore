...
export interface GenerateReflectionTextParams {
  conversation: string;
...
function getExtensionApiImportSpecifiers(extensionApiPath?: string): string[] {
  const requireFromHere = createRequire(import.meta.url);
  const specifiers: string[] = [];
  const configuredPath = extensionApiPath?.trim() || process.env.OPENCLAW_EXTENSION_API_PATH?.trim();
  if (configuredPath) specifiers.push(toImportSpecifier(configuredPath));
...
export function buildReflectionPrompt(
  ...
  const errorHints = toolErrorSignals.length > 0
    ? toolErrorSignals
      .map((signal, index) =>
        `${index + 1}. [${signal.toolName}] ${signal.summary} (sig:${signal.signatureHash.slice(0, 8)})`,
      )
      .join("\n")
    : "- (none)";
...
export function createReflectionTextGenerator(dependencies: ReflectionGeneratorDependencies) {
  let embeddedRunnerPromise: Promise<EmbeddedPiRunner> | null = null;
  const now = dependencies.now ?? Date.now;
  const random = dependencies.random ?? Math.random;

  const loadEmbeddedRunner = async (): Promise<EmbeddedPiRunner> => {
    if (dependencies.loadEmbeddedRunner) return dependencies.loadEmbeddedRunner();
      ...
      embeddedRunnerPromise = (async () => {
        ...
        for (const specifier of getExtensionApiImportSpecifiers(dependencies.extensionApiPath)) {
          try {
            ...
            const runner = (module as Record<string, unknown>).runEmbeddedPiAgent;
            if (typeof runner === "function") return runner as EmbeddedPiRunner;
            errors.push(`${specifier}: runEmbeddedPiAgent export not found`);
          } catch (error) {
            errors.push(
              `candidate=${dependencies.diagnosticIdentifier(specifier)} ` +
  ...
  return async function generateReflectionText(
    ...
    const tempSessionFile = join(
      tmpdir(),
      `memory-reflection-${now()}-${random().toString(36).slice(2)}.jsonl`,
    );
    ...
    try {
      ...
      const payloads = result && typeof result === "object" && Array.isArray(
        (result as Record<string, unknown>).payloads,
      )
        ? (result as Record<string, unknown>).payloads as unknown[]
        : [];
      const firstWithText = payloads.find(
        (payload) =>
          ...
          ((payload as Record<string, unknown>).text as string).trim().length > 0,
      ) as Record<string, unknown> | undefined;
