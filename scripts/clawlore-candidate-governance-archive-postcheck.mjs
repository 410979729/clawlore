...
const { inspectLiveCandidateGovernanceArchiveV1 } = jiti(
...
function parseArgs(argv) {
  ...
  for (const required of ["source", "plan", "apply-receipt", "plan-digest", "output"]) {
...
await writeFile(resolve(args.output), `${JSON.stringify(postcheck, null, 2)}\n`, { flag: "wx", mode: 0o600 });
process.stdout.write(`${JSON.stringify({ status: postcheck.status, archivedRows: postcheck.targetBinding.archivedRows })}\n`);
