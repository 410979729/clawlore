...
const { acceptLiveCandidateGovernanceArchivePlanV1 } = jiti(
...
function parseArgs(argv) {
  ...
  for (const required of ["source", "plan", "plan-digest", "output"]) {
...
await writeFile(resolve(args.output), `${JSON.stringify(acceptance, null, 2)}\n`, { flag: "wx", mode: 0o600 });
process.stdout.write(`${JSON.stringify({ status: acceptance.status, planDigest: acceptance.planDigest })}\n`);
