...
function parseArgs(argv) {
  ...
  for (const required of [
    "source", "prior-baseline", "archive-plan", "apply-receipt", "postcheck",
...
const plan = createLivePostUnsafeTraceArchiveCandidatePlanV1({
  sourcePath: resolve(args.source),
  priorBaselinePath: resolve(args["prior-baseline"]),
  archivePlanPath: resolve(args["archive-plan"]),
...
process.stdout.write(`${JSON.stringify({
  ...
  unsafeTraceArchiveRebase: plan.unsafeTraceArchiveRebase,
